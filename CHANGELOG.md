...
- **SSO Receiver pada Shared Cache Store `sso`:** Sesi "Buka Aplikasi" dari holding kini membaca token pada cache store khusus `sso`, bukan store default aplikasi (`docs/SSO-CONTRACT.md`, bagian *Shared SSO Cache*).
  ...
  - `HoldingSsoController::consumePayload()` berubah dari `Cache::pull()` menjadi `Cache::store('sso')->pull()` — satu-satunya perubahan logika; verifikasi payload, validasi signature HMAC, dan tenant-binding tidak disentuh, konsumsi tetap sekali pakai.
  - `docker-compose.yml` menambahkan `extra_hosts: host.docker.internal:host-gateway` pada service `queue` (service `app` sudah memilikinya) agar container menjangkau bus redis holding di server yang sama. Client redis kedua aplikasi sudah `predis` (`REDIS_CLIENT`); connection `sso` membawa penanda `SSO_REDIS_CLIENT` agar niatnya eksplisit.
...
- **Adaptasi Pipeline Migrasi Legacy ke Skema SIMAK:**
  ...
  - Pembuatan tenant otomatis (`autoProvisionTenant`) mengonfigurasi `coa_variant` sesuai `jenis_akun` legacy dan menderivasi kode tenant dari `kd_desa`.
...
- **Strip 3 Modul — Anggota (Membership), Master Data, Tagihan (Billing Tenant):**
  ...
  - Pipeline migrasi data legacy keanggotaan (`app/Domain/Migration/Membership/`, commands `legacy:migrate-membership`, `legacy:discover-membership`, `legacy:sync-villages`) dan test `MigrateLegacyMembershipTest` ikut dihapus.
  - Controllers & requests: `MasterData/{MemberController,GroupController}` + `MemberRequest, QuickMemberRequest, GroupRequest`; Portal Anggota (`Portal/PortalMemberController`, route `/portal`, page `Portal/Index.vue`, permission `portal.self`).
  - Routes: blok `master-data.members.*`, `master-data.groups.*`, alias `membership.*`, `portal` — total 53 route dihapus (442 → 389), tanpa penambahan route (nesting aman).
  - Referensi silang dibersihkan: `TenantSnapshotService`/`DesktopPushApplyService` (table list sync), `TenantOnboardingService` (template anggota/kelompok), `AssistantToolService` (search_members/search_groups/groupMemberItems) + handlers `SearchMembersHandler`/`SearchGroupsHandler` + `config/permissions.php` (tool_map), `GlobalSearchService`, `DashboardService` (kartu Anggota Aktif → counts stub 0), `TenantUserManagementController` + `StoreTenantUserRequest`/`UpdateTenantUserRequest` (link user↔anggota), `RegencyConsolidatedReportService` (hitungan groups/members → 0), `InitializeTenantSequences`, `DesktopStatusCommand`, `BootstrapLocalEnvironment`/`TenantRegistrationService` (`TenantGroupMasterDataProvisioner` → `TenantMasterDataProvisioner`), `TenantCutoverRunnerService` + `Admin/MigrationController` (step skip_membership), sidebar `AuthenticatedLayout.vue`, `useKeyboardShortcuts`/`KeyboardShortcutsModal`, halaman Admin Migration.
  - Migrasi: `2026_09_02_100001_create_member_user_links_table.php` dan `2026_09_03_000001_add_identity_photo_to_people.php` dihapus. Migration gabungan `2026_07_18_100002_create_membership_and_group_tables.php` DIPERTAHANKAN utuh karena membuat `organization_profiles`, `organization_units`, `people`, dan tabel lookup `business_types`/`activity_types`/`group_levels`/`group_functions` yang masih dipakai modul lain (report, provisioning, desktop sync) — tabel `members`/`groups` kini tidak lagi dikonsumsi modul (laporan konsolidasi membaca angka 0), dipertahankan sesuai aturan "migration masih dipakai = jangan dihapus".
  - Tests dihapus: `MasterData/{MemberTest,MemberIdentityPhotoTest,GroupTest,EntityDetailTest,MasterDataTableTest}`, `Portal/PortalMemberTest`, `Search/GlobalSearchTest`, `Api/DesktopSyncApiTest` (push berbasis tabel member), `Admin/MigrationFlowTest`.
  - **B. Master Data (UI):** menghapus Pages `MasterData/{Villages,Institutions}` + routes + `VillageController`, `OtherInstitutionController`, `VillageRequest`, `OtherInstitutionRequest`, `OrganizationUnitRequest`. Model `OrganizationUnit` & `VillageNaming` DIPERTAHANKAN (tenant provisioning, asset relation, ProfitAllocationService).
  - **C. Tagihan (Billing Tenant):** dianalisis bahwa UI tagihan tenant (`Billing/InvoiceController`, routes `billing.invoices.*`, pages `Billing/Invoices`) adalah sisi tenant dari PENAGIHAN LANGGANAN PLATFORM (Platform\Invoice + SubscriptionGateService + webhook Tripay/Duitku/Xendit + Admin InvoiceController) — bukan modul tagihan operasional tenant. Keseluruhan area ini DIPERTAHANKAN sebagai infrastruktur langganan: middleware `EnsureSubscriptionActive` kini me-redirect tenant terblokir ke `dashboard` (bukan halaman invoice), notifikasi center, dan unpaid-invoice banner dashboard tetap berfungsi mengarah ke penagihan langganan. Test yang tersisa: `SubscriptionAutomationAndEnforcementTest`, `SubscriptionGateSyncTest`.
  - Sidebar: item "Daftar Tagihan", grup "Master Data", dan "Portal Saya" dihapus dari `AuthenticatedLayout.vue`; `NotificationDropdown` mengubah link sekunder Tagihan → Dashboard.
  - Sweep akhir: sisa kata anggota/member di `app/` hanya `App\Models\Platform\TenantMembership` (platform users, sah) + kolom teks laporan; sisa di frontend: `Admin/Tenants/Index.vue` (kolom memberships_count), teks pemasaran `Home.vue`, placeholder asisten.
