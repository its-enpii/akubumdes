# Daftar Bug & Patch — Akubumdes Audit 2026-08-14/15
...
**Root cause:** `akubumdes-queue-1` start **sebelum** `.env` di-update
...
**Fix:** `docker restart akubumdes-queue-1` agar worker me-load `.env`
terbaru.
...
**Re-verify:**
- `npm run build` di `akubumdes-node-1` → sukses tanpa error.
...
**Re-verify:**
- `node -e "..."` di `akubumdes-node-1` dengan 22 case → 22/22 PASS.
