...
const statusTone = computed(() =>
  props.budget.status === "approved" ? "success" : "warning",
);
const statusLabel = computed(() =>
  props.budget.status === "approved" ? "Disetujui" : "Draft",
);
...
function copyPrevious() {
  router.post(
    `/budgeting/${props.year}/${props.month}/copy-previous`,
    {},
    { preserveScroll: true },
  );
}
...
function monthMeta(month) {
  return (
    props.months.find((item) => item.month === month) || {
      line_count: 0,
      revenue: 0,
      expense: 0,
      surplus: 0,
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-7xl space-y-6 pb-28">
      ...
      >
        <div>
          ...
          <p class="mt-1 text-on-surface-variant">
            Rencana anggaran per tahun &amp; bulan. Hanya akun pendapatan/beban
            yang postable.
          </p>
        </div>
      ...
      <div class="grid gap-2 sm:grid-cols-3 lg:grid-cols-6 xl:grid-cols-12">
        <AppButton
          ...
          @click="selectMonth(m)"
        >
          <span class="text-xs font-bold uppercase tracking-wide opacity-80">{{
            monthLabels[m]?.slice(0, 3)
          }}</span>
          <span class="mt-1 text-sm font-semibold">
            {{
              monthMeta(m).line_count > 0
                ? formatMoney(monthMeta(m).surplus)
                : "—"
            }}
          </span>
        </AppButton>
      ...
      <div class="grid gap-4 md:grid-cols-3">
        <AppCard>
          ...
          <p class="mt-2 text-2xl font-bold text-primary">
            {{ formatMoney(liveTotals.revenue) }}
          </p>
        </AppCard>
        <AppCard>
          <p
          ...
          <p class="mt-2 text-2xl font-bold text-primary">
            {{ formatMoney(liveTotals.expense) }}
          </p>
        </AppCard>
        <AppCard>
          <p
          ...
          <p
            ...
            :class="liveTotals.surplus >= 0 ? 'text-primary' : 'text-error'"
          >
      ...
      <div
        v-if="!editable"
        class="rounded-xl border border-outline-variant bg-surface-container-low px-4 py-3 text-sm text-on-surface-variant"
      >
    ...
    >
      <div
        class="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 sm:px-6 sm:flex-row sm:items-center sm:justify-between lg:px-8"
      >
        <div class="min-w-0 text-sm text-on-surface-variant">
          <span class="font-semibold text-primary"
            >{{ monthLabels[month] }} {{ year }}</span
          >
          <span class="mx-2 text-outline">·</span>
          Surplus {{ formatMoney(liveTotals.surplus) }}
        </div>
        <div
          v-if="can('budgeting.manage')"
