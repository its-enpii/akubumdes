...
async function destroy() {
  if (
    !(await confirmAction({
      ...
      message: `Hapus inventaris "${props.asset.name}"?`,
    }))
  )
    return;
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      >
        ...
        <div class="flex flex-wrap gap-2">
          <Link href="/accounting/assets"
            ><AppButton variant="ghost" icon="arrow_back"
              >Daftar</AppButton
            ></Link
          >
          <Link
            v-if="can('assets.manage')"
            :href="`/accounting/assets/${asset.row_id}/edit`"
            ><AppButton icon="edit">Edit</AppButton></Link
          >
          <AppButton
            v-if="can('assets.manage')"
            variant="ghost"
      ...
      <AppCard>
        <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-primary">
          Detail
        </h2>
        <dl class="grid gap-3 sm:grid-cols-2 text-sm">
          ...
          <div>
            ...
            <dd class="font-medium">
              {{ asset.quantity }} × {{ formatMoney(asset.unit_cost) }}
            </dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd class="font-medium">
              {{ asset.age_months }} bulan · susut/bln
        ...
        <ul v-else class="divide-y divide-outline-variant/20">
          <li v-for="h in histories" :key="h.row_id" class="px-4 py-3 text-sm">
            <div class="flex flex-wrap items-baseline justify-between gap-2">
              <p class="font-medium">
                <span v-if="h.from_label" class="text-on-surface-variant"
                  >{{ h.from_label }} →
                </span>
                {{ h.to_label }}
              </p>
            ...
            <p v-if="h.notes" class="mt-1 text-on-surface-variant">
              {{ h.notes }}
            </p>
          </li>
        </ul>
