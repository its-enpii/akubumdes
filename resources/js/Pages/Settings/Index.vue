...
const props = defineProps({
  ...
  offline: {
    type: Object,
    default: () => ({ is_enabled: false, user_id: null, users: [] }),
  },
});
...
const offlineUserOptions = computed(() =>
  (props.offline.users ?? []).map((u) => ({
    value: u.row_id,
...
// === Signatures ===
const signatureReportKey = ref(
  props.signatures.reportTypes?.[0]?.key ?? "default",
);
...
const signatureReportOptions = computed(() =>
  (props.signatures.reportTypes ?? []).map((t) => ({
    value: t.key,
    label: t.label,
  })),
);
...
const currentSignatureImageUrl = computed(
  () => props.signatureImages?.[signatureReportKey.value] ?? null,
);
...
const signatureImageFile = computed({
  get: () => signatureImageForm.image,
  set: (file) => {
    if (!file) return;
    ...
    <div class="mx-auto max-w-7xl space-y-6">
      <header>
        ...
        <p class="mt-1 text-on-surface-variant">
          Konfigurasi lembaga, logo, WhatsApp, dan tanda tangan.
        </p>
      </header>

      <div class="grid gap-6 lg:grid-cols-[14rem_1fr]">
        <div
          class="rounded-xl bg-surface-container-low p-2 lg:sticky lg:top-20 lg:self-start"
        >
          <AppTabs
            v-model="activeTab"
            :items="tabs"
        ...
        <div class="space-y-6">
          <AppCard v-show="activeTab === 'identity'" bordered>
            <h2 class="mb-1 text-lg font-bold text-primary">
              Identitas Lembaga
            </h2>
            <p class="mb-5 text-sm text-on-surface-variant">
              Profil hukum dan kontak lembaga. Data ini muncul pada laporan dan
              dokumen resmi.
            </p>
            <form class="space-y-5" @submit.prevent="submitIdentity">
              <div class="grid gap-4 sm:grid-cols-2">
                <AppInput
                  v-model="identityForm.legal_name"
                  label="Nama Legal"
                  required
                  :error="identityForm.errors.legal_name"
                />
                <AppInput
                  v-model="identityForm.short_name"
                  label="Nama Pendek"
                  :error="identityForm.errors.short_name"
                />
                <AppInput
                  v-model="identityForm.registration_number"
                  label="Nomor Badan Hukum"
                  :error="identityForm.errors.registration_number"
                />
                <AppInput
                  v-model="identityForm.tax_number"
                  label="NPWP"
                  :error="identityForm.errors.tax_number"
                />
                <AppInput
                ...
                <AppInput
                  ...
                  label="Website"
                  type="url"
                  :error="identityForm.errors.website"
                  class="sm:col-span-2"
                />
                <AppTextarea
                ...
                />
                <AppInput
                  v-model="identityForm.timezone"
                  label="Zona Waktu"
                  required
                  :error="identityForm.errors.timezone"
                />
                <AppDatePicker
                  v-model="identityForm.operational_start_date"
                  label="Tanggal Operasional Mulai"
                  :error="identityForm.errors.operational_start_date"
                />
              </div>
              <div
          ...
          <AppCard v-show="activeTab === 'logo'" bordered>
            ...
            <p class="mb-5 text-sm text-on-surface-variant">
              Logo akan tampil di sidebar aplikasi. Format: PNG, JPG, atau WebP.
              Maks 2 MB.
            </p>
            <div class="flex flex-col items-center gap-5">
              <div
                class="grid size-40 place-items-center overflow-hidden rounded-2xl border border-outline-variant bg-surface-container-lowest"
              >
                <img
                  v-if="logoPreview"
                  :src="logoPreview"
              ...
              <form class="w-full space-y-3" @submit.prevent="submitLogo">
                <div
                  ...
                  :class="
                    logoDragOver ? 'border-primary bg-primary-container/20' : ''
                  "
                ...
                >
                  ...
                  <p class="mt-2 text-sm font-bold text-primary">
                    Tarik gambar ke sini atau klik untuk pilih
                  </p>
          ...
          <AppCard v-show="activeTab === 'signatures'" bordered>
            ...
            <div
              class="mb-5 flex items-start gap-3 rounded-xl border border-outline-variant bg-surface-container-low p-4"
            >
              <AppIcon name="info" class="mt-0.5 text-primary" />
              <p class="text-sm text-on-surface-variant">
                Tanda tangan gambar akan otomatis disisipkan ke PDF melalui
                placeholder
                <code
                  class="rounded bg-surface-container px-1.5 py-0.5 font-mono text-xs"
                  >{ttd_image}</code
                >
            ...
            <div
              class="mb-6 rounded-xl border border-outline-variant bg-surface-container-low p-4"
            >
              ...
              <p
                v-if="signatureImageForm.errors.image"
                class="mt-2 text-sm text-error"
              >
                {{ signatureImageForm.errors.image }}
              </p>
              <p
                v-if="signatureDeleteForm.errors.report_key"
                class="mt-2 text-sm text-error"
              >
            ...
            <form class="space-y-5" @submit.prevent="submitSignatures">
              ...
              />
              <p
                v-if="signatureForm.errors.templates"
                class="text-sm text-error"
              >
                {{ signatureForm.errors.templates }}
              </p>
              <div
                class="flex justify-end border-t border-outline-variant pt-4"
              >
                <AppButton
                  type="submit"
                  icon="save"
          ...
          >
            <SignaturePad
              ref="signatureImagePad"
              @save="saveSignatureImage"
