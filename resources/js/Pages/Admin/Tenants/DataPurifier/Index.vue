...
const allSelected = computed(
  () =>
    props.rows.length > 0 &&
    props.rows.every((r) => selectedRowIds.value.includes(r.row_id)),
);
...
function startTraining() {
  startTrainingForm.post(
    `/admin/tenants/${props.tenant.row_id}/data-purifier/start-training`,
    {
      preserveScroll: true,
    },
  );
...
function submitEndTraining() {
  endTrainingForm.post(
    `/admin/tenants/${props.tenant.row_id}/data-purifier/end-training`,
    {
      preserveScroll: true,
      onSuccess: () => {
...
function submitResetTraining() {
  resetForm.post(
    `/admin/tenants/${props.tenant.row_id}/data-purifier/reset-training`,
    {
      preserveScroll: true,
      onSuccess: () => {
  ...
  <AdminLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        <div>
          <Link
            :href="`/admin/tenants/${tenant.row_id}`"
          ...
          <div class="mt-2 flex flex-wrap items-center gap-3">
            <h1 class="text-2xl font-bold text-primary">
              Data Purifier &amp; Siklus Pelatihan
            </h1>
          ...
          <p class="mt-1 text-sm text-on-surface-variant">
            Tenant:
            <span class="font-bold text-primary">{{ tenant.name }}</span> ({{
              tenant.code
            }}) · Isolasi dan pembersihan transaksi uji coba secara presisi.
          </p>
        </div>
      </header>
          ...
          <div class="space-y-1.5">
            <div class="flex items-center gap-2">
              <span
                ...
                :class="
                  tenant.is_training_mode
                    ? 'bg-warning animate-pulse'
                    : tenant.has_completed_training
                      ? 'bg-secondary'
                      : 'bg-outline'
                "
              />
              <h2 class="text-lg font-bold text-primary">
                Status:
                <span v-if="tenant.is_training_mode" class="text-warning"
                  >Mode Pelatihan Aktif</span
                >
                <span
                  v-else-if="tenant.has_completed_training"
                  class="text-secondary"
                  >Mode Live / Produksi (Pelatihan Selesai)</span
                >
                <span v-else class="text-on-surface-variant"
                  >Belum Memulai Sesi Pelatihan</span
                >
              </h2>
            </div>
            <p
              v-if="tenant.is_training_mode"
              class="text-xs text-on-surface-variant"
            >
              Sesi dimulai pada
              <span class="font-semibold text-primary">{{
                formatDate(tenant.training_started_at)
              }}</span
              >. Seluruh transaksi yang di-input pada periode ini tercatat
              sebagai data pelatihan (total:
              {{ stats.training_transactions_count }} transaksi).
            </p>
            <p
              v-else-if="tenant.has_completed_training"
              class="text-xs text-on-surface-variant"
            >
              ...
              <span class="font-semibold text-primary">{{
                formatDate(tenant.training_ended_at)
              }}</span
              >. Tenant saat ini beroperasi normal (Fitur reset transaksi
              terkunci demi keamanan data operasional).
            </p>
            <p v-else class="text-xs text-on-surface-variant">
              Aktifkan Mode Pelatihan sebelum sesi pelatihan dimulai agar sistem
              dapat mencatat dan mengisolasi seluruh transaksi latihan.
            </p>
          </div>
      ...
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <AppCard>
          ...
          <p class="mt-1 text-xs text-on-surface-variant">
            Keseluruhan di database shard
          </p>
        </AppCard>
        <AppCard>
          <p class="text-xs font-bold uppercase tracking-wider text-warning">
            Transaksi Sesi Pelatihan
          </p>
          <p class="mt-2 text-3xl font-bold text-warning">
            {{ stats.training_transactions_count }}
          </p>
          <p class="mt-1 text-xs text-on-surface-variant">
        ...
        <AppCard>
          ...
          <p class="mt-2 text-3xl font-bold text-secondary">
            {{ stats.opening_balances_count }}
          </p>
          <p class="mt-1 text-xs text-on-surface-variant">
            100% Terlindungi &amp; Utuh
          </p>
        </AppCard>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-bold text-primary">
              {{ selectedRowIds.length }} transaksi dipilih untuk di-purge
            </p>
            <p class="text-xs text-on-surface-variant">
              Total nominal: {{ money(selectedTotalAmount) }}
            </p>
          </div>
        </div>
        <div class="flex items-center gap-3">
      ...
      <AppCard :padded="false">
        <div class="overflow-x-auto">
          <ReportTable
            ...
            row-key="row_id"
            size="compact"
            empty-title="Tidak ada transaksi yang sesuai filter."
          >
            ...
            <template #cell-category="{ row }"
              ><AppBadge v-if="row.is_training" tone="warning"
                >Sesi Pelatihan</AppBadge
              ><AppBadge v-else-if="row.is_legacy" tone="neutral"
                >Legacy Migrasi</AppBadge
              ><AppBadge v-else tone="success">Produksi</AppBadge></template
            >
            ...
            <template #cell-description="{ row }">
              ...
              <p
                v-else-if="row.already_reversed && row.reversal_id"
                class="mt-0.5 text-xs text-error"
              >
                Telah dibatalkan oleh jurnal
                <span class="font-semibold">#{{ row.reversal_id }}</span>
              </p>
            </template>
        ...
        >
          <p class="text-on-surface-variant">
            {{ pagination.total }} transaksi · hlm {{ pagination.page }}/{{
      ...
      <AppModal
        v-model="purgeModalOpen"
        title="Konfirmasi Hard Delete (Purge Transaksi)"
      >
        <form class="space-y-4" @submit.prevent="submitPurge">
          <div
            class="rounded-xl border border-error/40 bg-error-container/20 p-4"
          >
            <p class="text-sm font-semibold text-error">
              Peringatan: Penghapusan Permanen (Hard Delete)
            </p>
            <p class="mt-1 text-xs text-on-surface-variant">
              Sebanyak
              ...
              >
              akan <strong>dihapus bersih dari basis data</strong> (termasuk
              baris jurnal). Transaksi ini tidak akan muncul lagi di buku besar
      ...
      <AppModal v-model="resetModalOpen" title="Reset Transaksi Sesi Pelatihan">
        <form class="space-y-4" @submit.prevent="submitResetTraining">
          ...
          >
            <p class="text-sm font-semibold text-error">
              Pembersihan Transaksi Sesi Pelatihan
            </p>
            <p class="mt-1 text-xs text-on-surface-variant">
              Sistem akan menghapus seluruh
              <span class="font-bold text-primary"
                >{{ stats.training_transactions_count }} transaksi</span
              >
              yang dibuat sejak sesi pelatihan dimulai (<span
                class="font-semibold"
                >{{ formatDate(tenant.training_started_at) }}</span
              >).
            </p>
          </div>

          <div
            class="rounded-xl bg-surface-container-low p-3 text-xs text-on-surface-variant space-y-1.5"
          >
            <p class="font-bold text-secondary">
              Data yang TETAP UTUH &amp; TERLINDUNGI:
            </p>
          ...
          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-primary">
              Ketik
              <code
                class="rounded bg-error-container px-1 py-0.5 text-error font-bold"
                >RESET</code
              >
              untuk konfirmasi:
            </label>
      ...
      <AppModal
        v-model="endTrainingModalOpen"
        title="Selesaikan Pelatihan &amp; Masuk Mode Live"
      >
        <form class="space-y-4" @submit.prevent="submitEndTraining">
          <div
            class="rounded-xl border border-secondary/40 bg-secondary-container/20 p-4"
          >
            <p class="text-sm font-semibold text-secondary">
              Beralih ke Mode Live / Produksi
            </p>
            <p class="mt-1 text-xs text-on-surface-variant">
              Setelah masuk Mode Live, tenant akan beroperasi secara resmi dan
              fitur reset pelatihan akan <strong>dikunci otomatis</strong> demi
              keamanan.
            </p>
          </div>
          ...
          <div class="rounded-xl bg-surface-container-low p-4 space-y-3">
            <div
              class="flex items-start gap-3 text-sm font-medium text-primary"
            >
              <AppCheckbox
                v-model="endTrainingForm.purge_data"
                label="Bersihkan transaksi pelatihan sekarang"
                variant="inline"
              />
              <div>
                <p class="font-bold">
                  Bersihkan {{ stats.training_transactions_count }} transaksi
                  pelatihan sekarang
                </p>
                <p class="text-xs text-on-surface-variant">
                  Centang untuk memulai masa live dengan buku besar yang bersih
                  dari transaksi coba-coba saat latihan.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-2">
            <AppButton
              type="button"
              variant="secondary"
              @click="endTrainingModalOpen = false"
              >Batal</AppButton
            >
            <AppButton
              type="submit"
