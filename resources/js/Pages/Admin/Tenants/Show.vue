...
const subForm = useForm({
  ...
  status: "active",
  starts_at:
    props.tenant.active_subscription?.starts_at ||
...
function money(value, currency = "IDR") {
  const { money: formatCurrency } = useMoneyFormatter();
  return formatCurrency(value, currency);
}

function assignPlan() {
  subForm.post(`/admin/tenants/${props.tenant.row_id}/subscription`, {
    preserveScroll: true,
  });
}

function suspend() {
  suspendForm.post(`/admin/tenants/${props.tenant.row_id}/suspend`, {
    preserveScroll: true,
  });
}

function activate() {
  activateForm.post(`/admin/tenants/${props.tenant.row_id}/activate`, {
    preserveScroll: true,
  });
}

function repair() {
  repairForm.post(`/admin/tenants/${props.tenant.row_id}/repair`, {
    preserveScroll: true,
  });
}
...
function generateInvoice() {
  ...
  invoiceForm.post(
    `/admin/subscriptions/${props.tenant.active_subscription.row_id}/invoices`,
    { preserveScroll: true },
  );
}
...
async function autoLogin(domain = null, userId = null) {
  ...
  try {
    const response = await fetch(
      `/admin/tenants/${props.tenant.row_id}/impersonate`,
      {
        method: "POST",
        headers: {
...
<template>
  ...
  <AdminLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        <div>
          <Link href="/admin/tenants" class="text-sm font-semibold text-primary"
            >← Daftar tenant</Link
          >
          ...
          <p class="mt-1 text-on-surface-variant">
            {{ tenant.code }} · kecamatan {{ tenant.district_code || "—" }}
          </p>
        </div>
        <div class="flex flex-wrap gap-2">
          ...
          <Link :href="`/admin/tenants/${tenant.row_id}/edit`"
            ><AppButton variant="secondary" icon="edit">Edit</AppButton></Link
          >
          <Link :href="`/admin/tenants/${tenant.row_id}/users`"
            ><AppButton variant="secondary" icon="group">Users</AppButton></Link
          >
          <Link :href="`/admin/tenants/${tenant.row_id}/onboarding/import`"
            ><AppButton variant="secondary" icon="account_balance_wallet"
              >Onboarding / Saldo Awal</AppButton
            ></Link
          >
          <Link :href="`/admin/tenants/${tenant.row_id}/data-purifier`"
            ><AppButton variant="secondary" icon="cleaning_services"
              >Data Purifier</AppButton
            ></Link
          >
          <Link :href="`/admin/invoices/create?tenant_id=${tenant.row_id}`"
            ><AppButton variant="secondary" icon="receipt_long"
              >Buat Invoice</AppButton
            ></Link
          >
          <AppButton
            variant="secondary"
            icon="build"
            :loading="repairForm.processing"
            @click="repair"
            >Lengkapi provision</AppButton
          >
          <AppButton
      ...
      <div class="grid gap-6 xl:grid-cols-3">
        <AppCard class="xl:col-span-2">
          ...
          <dl class="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              <dt class="text-sm text-on-surface-variant">Shard</dt>
              <dd class="font-semibold text-primary">
                {{ tenant.placement?.shard?.code || "—" }}
              </dd>
            </div>
            <div>
              <dt class="text-sm text-on-surface-variant">Database</dt>
              <dd class="font-semibold text-primary">
                {{ tenant.placement?.shard?.database_name || "—" }}
              </dd>
            </div>
            <div>
              <dt class="text-sm text-on-surface-variant">Provisioned</dt>
              <dd class="font-semibold text-primary">
                {{ tenant.provisioned_at || "—" }}
              </dd>
            </div>
            <div>
              <dt class="text-sm text-on-surface-variant">Suspended</dt>
              <dd class="font-semibold text-primary">
                {{ tenant.suspended_at || "—" }}
              </dd>
            </div>
            <div>
              <dt class="text-sm text-on-surface-variant">Koordinat Peta</dt>
              <dd class="font-semibold text-primary font-mono text-xs">
                {{
                  tenant.map_latitude && tenant.map_longitude
                    ? tenant.map_latitude +
                      ", " +
                      tenant.map_longitude +
                      " (Zoom: " +
                      (tenant.map_zoom || 13) +
                      ")"
                    : "Belum diatur"
          ...
          <div class="mt-6 border-t border-outline-variant pt-4">
            <div class="flex items-center justify-between">
              <dt class="text-sm font-semibold text-primary">
                Custom Domain Terdaftar
              </dt>
              <Link
                :href="`/admin/tenants/${tenant.row_id}/edit`"
            ...
            <div
              v-if="tenant.custom_domains && tenant.custom_domains.length > 0"
            ...
            <p v-else class="mt-1 text-xs text-on-surface-variant">
              Belum ada custom domain. Tenant diakses via subdomain default atau
              route identifier.
            </p>
          </div>
        </AppCard>
        ...
        <AppCard>
          ...
          <div v-if="tenant.active_subscription" class="mt-3 space-y-1">
            ...
            <p class="text-sm text-on-surface-variant">
              ...
              {{ tenant.active_subscription.starts_at }} →
          ...
          <form
            class="mt-5 space-y-3 border-t border-outline-variant pt-4"
          ...
          >
            <SmartSelect
              ...
              :options="
                plans.map((p) => ({
                  value: p.row_id,
                  label: `${p.name} (${money(p.price_amount, p.currency)}/${p.billing_period})`,
                }))
              "
            ...
            <AppButton
              type="submit"
              size="compact"
              :loading="subForm.processing"
              :disabled="!plans.length"
              >Simpan langganan</AppButton
            >
          </form>
        </AppCard>
      </div>
      ...
      <div class="grid gap-6 xl:grid-cols-2">
        <AppCard :padded="false">
          ...
          >
            ...
            <Link
              :href="`/admin/tenants/${tenant.row_id}/users`"
        ...
        <AppCard :padded="false">
          ...
          <ul class="divide-y divide-outline-variant">
            ...
            >
              <div>
                <Link
                  :href="`/admin/invoices/${invoice.row_id}`"
