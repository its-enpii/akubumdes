...
const testTripayConnection = async () => {
  ...
  try {
    const data = await apiCall("/admin/payment-gateways/tripay/test", {
      method: "POST",
    });
...
const setGateway = (gw) => {
  ...
  gatewayForm.post("/admin/payment-gateways/active", {
    preserveScroll: true,
    onSuccess: () =>
      showToast(`Payment Gateway utama diubah ke ${gw.toUpperCase()}`),
  });
};
...
const testXenditConnection = async () => {
  ...
  try {
    const data = await apiCall("/admin/payment-gateways/xendit/test", {
      method: "POST",
    });
...
const testDuitkuConnection = async () => {
  ...
  try {
    const data = await apiCall("/admin/payment-gateways/duitku/test", {
      method: "POST",
    });
...
async function testConnection() {
  ...
  try {
    testResult.value = await apiCall("/admin/ai-assistant/tools/sync", {
      method: "POST",
    });
  } catch (e) {
...
async function submitPersona() {
  ...
  const url =
    personaModal.value.mode === "create"
...
async function deletePersona(persona) {
  ...
  if (!ok) return;
  try {
    const data = await apiCall(`/admin/ai-assistant/personas/${persona.id}`, {
      method: "DELETE",
    });
    await refreshPersonas();
...
async function togglePersona(persona, field) {
  try {
    const data = await apiCall(
      `/admin/ai-assistant/personas/${persona.id}/toggle`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function refreshPersonas() {
  try {
    const data = await apiCall("/admin/ai-assistant/personas", {
      method: "GET",
    });
...
const personaToolOptions = computed(() =>
  tools.value.map((t) => ({
    value: t.id,
    label: t.name,
...
async function syncTools() {
  ...
  try {
    const data = await apiCall("/admin/ai-assistant/tools/sync", {
      method: "POST",
    });
...
const uploadPersonaOptions = computed(() => [
  {
    value: "",
    label: "— Tanpa persona (global) —",
    description: "Dokumen tersedia global",
  },
  ...personas.value.map((p) => ({
    value: p.id,
    label: p.name,
...
function setUploadFile(f) {
  ...
  if (
    ...
    !["txt", "md", "html", "pdf", "docx"].includes(ext)
  ) {
...
async function submitUpload() {
  ...
  try {
    ...
    uploadResult.value = {
      ok: true,
      message: `Berhasil ingest: ${data.document.title}`,
    };
...
async function viewDocument(doc) {
  try {
    const data = await apiCall(`/admin/ai-assistant/documents/${doc.id}`, {
      method: "GET",
    });
...
const chatPersonaOptions = computed(() => [
  {
    ...
    description: "Gunakan persona default sistem",
  },
  ...personas.value
...
function handleChatEvent(event, data, assistantMsg) {
  ...
  if (event === "confirmation_required") {
    ...
    pushChat({
      role: "assistant",
      content: `⚠️ï¸ **Konfirmasi diperlukan:** ${summary}`,
    });
    return;
  }
  if (event === "error") {
...
async function sendChatTest(content) {
  ...
  try {
    const res = await fetch("/admin/ai-assistant/chat", {
      ...
      body: JSON.stringify({
        message: content,
        persona_slug: chatPersonaId.value
          ? personas.value.find((p) => p.id === chatPersonaId.value)?.slug
          : null,
      }),
      signal: chatAbort.signal,
    });
    await readSseStream(res, (event, data) =>
      handleChatEvent(event, data, assistantMsg),
    );
    if (!assistantMsg.id) {
      assistantMsg.id = chatMessages.value.length + 1;
      pushChat({
        role: "assistant",
        content: assistantMsg.content || "Tidak ada respon.",
      });
    }
  } catch (e) {
...
async function fetchConversations() {
  ...
  try {
    const data = await apiCall("/admin/ai-assistant/conversations", {
      method: "GET",
    });
    conversations.value = {
      loading: false,
...
async function fetchAuditLogs() {
  ...
  try {
    const data = await apiCall("/admin/ai-assistant/audit-logs", {
      method: "GET",
    });
...
const toolColumns = [
  { key: "name", label: "Tool" },
  { key: "registry", label: "Registry", align: "center" },
  { key: "confirmation", label: "Konfirmasi", align: "center" },
...
const docColumns = [
  ...
  { key: "content_length", label: "Ukuran", align: "right" },
  { key: "chunks_count", label: "Chunks", align: "right" },
  { key: "created_at", label: "Diupload" },
  { key: "actions", label: "Aksi", align: "right" },
];
const conversationColumns = [
  { key: "persona_name", label: "Persona" },
  { key: "channel", label: "Channel" },
  { key: "messages_count", label: "Pesan", align: "right" },
  { key: "started_at", label: "Mulai" },
  { key: "last_activity_at", label: "Aktivitas" },
];
const auditColumns = [
  ...
  <AdminLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      <header class="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-on-surface-variant">
            Kelola AI Personas, Tools, Knowledge Base (RAG), Live Test Chat, dan
            Monitor Aktivitas Log In-Process.
          </p>
        </div>
      ...
      <AppCard v-if="flash">
        <div class="flex items-center gap-3">
          <div
            class="grid size-10 shrink-0 place-items-center rounded-full bg-secondary-container text-secondary"
          >
            ✓
          </div>
          ...
          >
            <div>
              ...
              <p class="mt-0.5 text-xs text-on-surface-variant">
                System prompt + tool scope per persona.
              </p>
            </div>
            <AppButton icon="add" @click="openCreatePersona"
              >Persona Baru</AppButton
            >
          </header>

          <div
            v-if="personas.length === 0"
            class="rounded-xl border border-dashed border-outline-variant bg-surface-container-lowest px-4 py-10 text-center"
          >
            <AppIcon name="person" class="text-4xl text-on-surface-variant" />
            <p class="mt-2 text-sm text-on-surface-variant">
              Belum ada persona. Buat satu untuk mulai.
            </p>
          </div>
          <div
            v-else
            class="overflow-x-auto rounded-xl border border-outline-variant"
          >
            <ReportTable
            ...
            >
              <template #cell-name="{ row }">
                <div class="flex items-center gap-2">
                  ...
                  <AppBadge v-if="row.is_default" tone="primary"
                    >Default</AppBadge
                  >
                </div>
                <p class="mt-0.5 line-clamp-1 text-xs text-on-surface-variant">
                  {{ row.system_prompt }}
                </p>
              </template>
              <template #cell-tools_count="{ row }"
              ...
              <template #cell-actions="{ row }">
                <div class="flex items-center justify-end gap-1">
                  ...
                  <AppIconButton
                    ...
                    @click="togglePersona(row, 'is_default')"
                  />
                  <AppIconButton
                    :name="row.is_active ? 'toggle_on' : 'toggle_off'"
                    size="sm"
                    :tooltip="row.is_active ? 'Nonaktifkan' : 'Aktifkan'"
                    @click="togglePersona(row, 'is_active')"
                  />
                  <AppIconButton
                    v-if="!row.is_default"
                    name="delete"
      ...
      <div v-else-if="activeTab === 'tools'" class="space-y-6">
        <AppCard>
          ...
          >
            <div>
              <h2 class="text-lg font-bold text-primary">
                Tool Registry & Sinkronisasi
              </h2>
              <p class="mt-0.5 text-xs text-on-surface-variant">
                Tools ter-register via
                <code class="rounded bg-surface-container px-1 py-0.5 text-xs"
                  >ToolHandler</code
                >. Sync dari codebase untuk materialisasi DB row.
              </p>
            </div>
            <AppButton icon="sync" :loading="toolSyncing" @click="syncTools"
              >Sinkronkan dari Registry</AppButton
            >
          </header>
          ...
          >
            ...
            <p class="mt-2 text-sm text-on-surface-variant">
              Belum ada tool di DB. Klik "Sinkronkan" untuk menarik dari
              registry.
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              ...
              <template #cell-registry="{ row }"
                ><AppBadge :tone="row.is_registered ? 'success' : 'warning'">{{
                  row.is_registered ? "Terdaftar" : "Tertinggal"
                }}</AppBadge></template
              >
              <template #cell-confirmation="{ row }">
                <AppIconButton
                  size="sm"
                  :tone="row.requires_confirmation ? 'warning' : 'neutral'"
                  :name="row.requires_confirmation ? 'lock' : 'lock_open'"
                  :tooltip="
                    row.requires_confirmation
                      ? 'Wajib konfirmasi'
                      : 'Tanpa konfirmasi'
                  "
                  @click="toggleTool(row, 'requires_confirmation')"
                />
              </template>
              <template #cell-active="{ row }">
                <AppIconButton
                  ...
                  :name="row.is_active ? 'toggle_on' : 'toggle_off'"
                  :tooltip="row.is_active ? 'Aktif' : 'Nonaktif'"
                  @click="toggleTool(row, 'is_active')"
                />
              </template>
              <template #cell-actions="{ row }">
                <div class="flex items-center justify-end gap-1">
      ...
      <div v-else-if="activeTab === 'knowledge'" class="space-y-6">
        <AppCard>
          <header class="mb-4">
            ...
            <p class="mt-0.5 text-xs text-on-surface-variant">
              File akan di-chunk dan di-embed untuk retrieval.
            </p>
          </header>
          <form class="space-y-4" @submit.prevent="submitUpload">
            ...
            <div>
              <label
                class="mb-1 block text-sm font-bold uppercase tracking-wider text-primary"
                >File</label
              >
              <label
              ...
              >
                <AppIcon
                  name="upload_file"
                  class="text-2xl text-on-surface-variant"
                />
                ...
                <p class="text-xs text-on-surface-variant">
                  PDF / DOCX / MD / HTML / TXT — maks 20 MB
                </p>
                <AppFileUpload
            ...
            <p
              v-if="uploadResult"
              class="text-xs"
              :class="uploadResult.ok ? 'text-success' : 'text-error'"
            >
              {{ uploadResult.message }}
            </p>
            <div
              class="flex justify-end gap-2 border-t border-outline-variant pt-4"
            >
              <AppButton
                type="submit"
                icon="upload"
        ...
        <AppCard>
          ...
          >
            <div>
              ...
              <p class="mt-0.5 text-xs text-on-surface-variant">
                Total {{ docs.items.length }} dokumen aktif.
              </p>
            </div>
            <div class="flex items-center gap-2">
              <SmartSelect
                v-model="uploadForm.persona_id"
                :options="kbPersonaOptions"
          ...
          <div
            v-if="docs.loading"
            class="rounded-xl bg-surface-container-lowest px-4 py-8 text-center text-sm text-on-surface-variant"
          >
            Memuat…
          </div>
          <AppCard
            v-else-if="docs.error"
            class="border-tertiary bg-tertiary-fixed/30 text-tertiary"
            >{{ docs.error }}</AppCard
          >
          <AppEmptyState
            ...
            description="Upload file di atas untuk mulai membangun knowledge base."
          />
          <div
            v-else
            class="overflow-x-auto rounded-xl border border-outline-variant"
          >
            ...
            >
              <template #cell-title="{ row }">
                <p class="font-semibold text-primary">{{ row.title }}</p>
                <p
                  v-if="row.preview"
              ...
              <template #cell-format="{ row }"
                ><span
                  class="rounded bg-surface-container-lowest px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-on-surface-variant"
                  >{{ row.format || "—" }}</span
                ></template
              >
              <template #cell-actions="{ row }">
        ...
        <AppCard>
          ...
          >
            <div>
              ...
              <p class="mt-0.5 text-xs text-on-surface-variant">
                Streaming SSE ke AgentLoop in-process.
              </p>
            </div>
            <div class="flex items-center gap-2">
              <SmartSelect
                v-model="chatPersonaId"
                :options="chatPersonaOptions"
                hide-label
                placeholder="Pilih persona"
                class="min-w-56"
              />
          ...
          <div
            ref="chatListEl"
            class="min-h-96 max-h-[28rem] space-y-2 overflow-y-auto rounded-lg bg-surface p-3"
          >
            <p
              v-if="!chatMessages.length"
                ...
                <div v-else class="flex flex-col gap-2">
                  <template v-for="block in blocksFor(msg)" :key="block.id">
                    ...
                    <ArtifactCard
                      ...
                      :block="block"
                      @open="openArtifact(block)"
                    />
                    <ActionButton
                      v-else-if="block.type === 'button'"
                      :block="block"
                      @submit="
                        (payload) => onComponentSubmitTest(block, payload)
                      "
                    />
                    <PollCard
            ...
            >
              <div
                class="flex max-w-[85%] items-center gap-2 rounded-2xl rounded-bl-sm border border-outline-variant bg-surface-container-lowest px-3 py-2"
              >
                <span class="flex items-center gap-1">
                  <span
                    class="inline-block size-1.5 animate-bounce rounded-full bg-on-surface/40"
          ...
          >
            ...
            >
              <template #cell-channel="{ row }"
                ><AppBadge tone="neutral">{{ row.channel }}</AppBadge></template
              >
      ...
      <!-- Person modal -->
      <AppModal
        v-model="personaModal.open"
        :title="
          personaModal.mode === 'create' ? 'Buat Persona' : 'Edit Persona'
        "
        size="lg"
      >
        <div class="space-y-4">
          <AppInput
            v-model="personaModal.data.name"
            label="Nama"
            placeholder="Contoh: Koperasi Assistant"
          />
          <AppInput
            v-model="personaModal.data.slug"
            label="Slug"
            placeholder="Contoh: koperasi-assistant (otomatis dari nama jika kosong)"
          />
          <AppTextarea
            v-model="personaModal.data.system_prompt"
            label="System Prompt"
            placeholder="Deskripsikan kepribadian, gaya bahasa, dan batasan persona ini…"
            :rows="8"
          />
          <div>
            <label
              class="mb-2 block text-sm font-bold uppercase tracking-wider text-primary"
              >Tool Scope (kosongkan = semua tools)</label
            >
            <div
              class="grid max-h-56 grid-cols-1 gap-2 overflow-y-auto rounded-xl border border-outline-variant bg-surface-container-lowest p-3 sm:grid-cols-2"
            >
              <label
                v-for="t in tools"
                :key="t.id"
                class="flex items-start gap-2 rounded-lg p-2 hover:bg-surface-container-low"
              >
                <AppCheckbox
                  class="mt-1"
                  :value="t.id"
                  ...
                  @update:model-value="
                    (checked) => {
                      if (
                        checked &&
                        !personaModal.data.tool_ids.includes(t.id)
                      ) {
                        personaModal.data.tool_ids.push(t.id);
                      } else if (!checked) {
                        personaModal.data.tool_ids =
                          personaModal.data.tool_ids.filter(
                            (id) => id !== t.id,
                          );
                      }
                    }
                ...
                <div class="min-w-0 flex-1">
                  <p class="truncate font-mono text-xs font-bold text-primary">
                    {{ t.name }}
                  </p>
                  <p
                    class="mt-0.5 line-clamp-1 text-xs text-on-surface-variant"
          ...
          <div class="flex items-center gap-6">
            <AppSwitch
              v-model="personaModal.data.is_active"
              label="Aktif"
              description="Persona dapat dipilih di chat"
            />
            <AppSwitch
              v-model="personaModal.data.is_default"
              label="Default"
              description="Persona fallback"
            />
          </div>
        </div>
        <template #footer>
          <AppButton variant="ghost" @click="personaModal.open = false"
            >Batal</AppButton
          >
          <AppButton icon="save" @click="submitPersona">{{
            personaModal.mode === "create" ? "Buat" : "Simpan"
          }}</AppButton>
        </template>
      </AppModal>
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <p class="mt-1">
                <AppBadge
                  :tone="toolModal.tool.is_registered ? 'success' : 'warning'"
                  >{{
                    toolModal.tool.is_registered ? "Terdaftar" : "Tertinggal"
                  }}</AppBadge
                >
              </p>
            </div>
            <div>
              ...
              <p class="mt-1">
                <AppBadge
                  :tone="
                    toolModal.tool.requires_confirmation ? 'warning' : 'neutral'
                  "
                  >{{
                    toolModal.tool.requires_confirmation
                      ? "Wajib konfirmasi"
                      : "Otomatis"
                  }}</AppBadge
                >
          ...
          <div>
            ...
            <pre
              class="mt-2 max-h-80 overflow-auto rounded-xl border border-outline-variant bg-surface-container-lowest p-3 font-mono text-xs"
              >{{ JSON.stringify(toolModal.tool.json_schema, null, 2) }}</pre>
          </div>
        </div>
        <template #footer>
      ...
      <AppModal
        v-if="docModal.doc"
        v-model="docModal.open"
        :title="docModal.doc.title"
        size="lg"
      >
        <div class="space-y-4">
          ...
          <div>
            <p
              class="text-xs font-bold uppercase tracking-wider text-on-surface-variant"
            >
              Chunks ({{ docModal.doc.chunks.length }})
            </p>
            <div
              class="mt-2 max-h-96 space-y-2 overflow-y-auto rounded-xl border border-outline-variant bg-surface-container-lowest p-3"
            >
              <div
                v-for="chunk in docModal.doc.chunks"
                :key="chunk.id"
              ...
              >
                ...
                >
                  ...
                  <AppBadge
                    :tone="chunk.has_embedding ? 'success' : 'warning'"
                    >{{
      ...
      <!-- Artifact modal -->
      <ArtifactModal
        :block="activeArtifactTest"
        @close="activeArtifactTest = null"
      />
      ...
      <Transition name="fade">
        <div
          v-if="toast"
          class="fixed bottom-6 right-6 z-50 max-w-sm rounded-xl border p-4 shadow-lg"
          :class="
            toast.tone === 'error'
