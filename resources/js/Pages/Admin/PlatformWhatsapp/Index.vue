...
const stats = computed(() => ({
  ...
  connected: props.instances.filter((instance) => isConnected(instance.status))
    .length,
}));
...
function statusTone(status) {
  ...
  if (
    ["unconfigured", "missing", "disconnected", "close", "closed"].includes(
      value,
    )
  )
    return "warning";
...
async function createSession(instance) {
  ...
  try {
    const response = await requestJson(
      `/admin/whatsapp/${instance.row_id}/create-session`,
      { method: "POST" },
    );
...
async function refreshState(instance) {
  ...
  try {
    const response = await requestJson(
      `/admin/whatsapp/${instance.row_id}/state`,
    );
    qrCode.value = response.qr ?? null;
...
async function deleteSession(instance) {
  ...
  try {
    const response = await requestJson(
      `/admin/whatsapp/${instance.row_id}/delete-session`,
      { method: "DELETE" },
    );
    toast[response.success ? "success" : "error"](
      response.message || "Session dihapus.",
    );
  } catch (error) {
...
async function destroyInstance(instance) {
  ...
  useForm({}).delete(`/admin/whatsapp/${instance.row_id}`, {
    preserveScroll: true,
  });
}
...
async function setDefault(instance) {
  ...
  useForm({}).post(`/admin/whatsapp/${instance.row_id}/set-default`, {
    preserveScroll: true,
  });
}
...
async function sendTest() {
  ...
  try {
    const response = await requestJson(
      `/admin/whatsapp/${testInstance.value.row_id}/test`,
      {
        method: "POST",
        body: JSON.stringify({
    ...
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        <div>
          ...
          <p class="mt-1 text-sm text-on-surface-variant">
            Kelola nomor WhatsApp platform untuk OTP lupa password dan
            pengiriman system-level.
          </p>
        </div>
        ...
        >
          <div>
            <h2 class="text-lg font-bold text-primary">
              Daftar Instance Platform
            </h2>
            <p class="mt-1 text-sm text-on-surface-variant">
              Instance ini dipakai OTP lupa password sebelum fallback ke
              instance tenant.
            </p>
          </div>
          <AppButton
      ...
      >
        <form class="space-y-4" @submit.prevent="submitInstance">
          <AppInput
            ...
            label="Nama Instance"
            required
            placeholder="Contoh: OTP Platform 1"
      ...
      >
        <div class="space-y-4 text-center">
          <AppBadge :tone="statusTone(qrStatus)">{{
      ...
      <AppModal
        ...
        title="Uji Kirim WhatsApp Platform"
        size="sm"
      >
