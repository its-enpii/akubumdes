...
function tone(status) {
  ...
  if (status === "overdue" || status === "void" || status === "failed")
    return "error";
...
function initiateTripay() {
  tripayForm.post(`/admin/invoices/${props.invoice.row_id}/payments/tripay`, {
    preserveScroll: true,
  });
}

function toggleBlocking() {
  toggleBlockingForm.post(
    `/admin/invoices/${props.invoice.row_id}/toggle-blocking`,
    { preserveScroll: true },
  );
}
...
async function voidInvoice() {
  if (
    !(await confirmAction({
      ...
      message: "Batalkan invoice ini? Tindakan ini tidak dapat dibatalkan.",
    }))
  )
    return;
  voidForm.post(`/admin/invoices/${props.invoice.row_id}/void`, {
    preserveScroll: true,
  });
}
    ...
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        <div>
          ...
          <div class="mt-3 flex flex-wrap items-center gap-3">
            ...
            <span
              v-if="invoice.blocks_access"
              class="rounded bg-error/15 px-2 py-0.5 text-xs font-bold text-error"
            >
          ...
          <p class="mt-1 text-on-surface-variant">
            <Link
              v-if="invoice.tenant"
              :href="`/admin/tenants/${invoice.tenant.row_id}`"
              class="font-semibold text-primary"
              >{{ invoice.tenant.name }}</Link
            >
            <span v-else>—</span>
      ...
      >
        <div class="flex items-center gap-3">
          <AppIcon
            :name="invoice.blocks_access ? 'block' : 'check_circle'"
            :class="
              invoice.blocks_access ? 'text-error' : 'text-on-surface-variant'
            "
            class="text-2xl"
          />
          <div>
            <h4
              ...
              :class="invoice.blocks_access ? 'text-error' : 'text-primary'"
            >
              {{
                invoice.blocks_access
                  ? "Opsi Blokir Akses: AKTIF"
                  : "Opsi Blokir Akses: Nonaktif"
              }}
            </h4>
            <p class="text-xs text-on-surface-variant">
              {{
                invoice.blocks_access
                  ? "Akses pengguna tenant diblokir sampai invoice ini dilunasi."
                  : "Tenant tetap dapat menggunakan aplikasi walau invoice ini belum dibayar."
              }}
            </p>
          </div>
        </div>
        <form v-if="invoice.is_open" @submit.prevent="toggleBlocking">
          <AppButton
            size="compact"
            :variant="invoice.blocks_access ? 'secondary' : 'danger'"
            :loading="toggleBlockingForm.processing"
          >
            {{
              invoice.blocks_access
                ? "Nonaktifkan Blokir"
                : "Aktifkan Blokir Akses"
            }}
          </AppButton>
        </form>
      </div>
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        <AppCard class="lg:col-span-2">
          ...
          <dl class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <dt class="text-sm text-on-surface-variant">Dibuat oleh</dt>
              <dd class="font-semibold text-primary">
                {{ invoice.creator?.name || "Sistem" }}
              </dd>
            </div>
            <div v-if="invoice.description" class="sm:col-span-2">
              <dt class="text-sm text-on-surface-variant">Deskripsi</dt>
              <dd class="text-primary">{{ invoice.description }}</dd>
            </div>
            <div v-if="invoice.notes" class="sm:col-span-2">
              <dt class="text-sm text-on-surface-variant">Catatan internal</dt>
              <dd class="whitespace-pre-line text-sm text-primary">
                {{ invoice.notes }}
              </dd>
            </div>
          </dl>
        ...
        <AppCard v-if="invoice.is_open" class="space-y-4">
          ...
          <form class="space-y-3" @submit.prevent="recordManual">
            ...
            <AppInput
              ...
              :error="manualForm.errors.reference"
            />
            <AppTextarea
              v-model="manualForm.notes"
              label="Catatan"
              :error="manualForm.errors.notes"
            />
            <AppButton
              type="submit"
              class="w-full"
          ...
          <div class="pt-4 border-t border-outline-variant">
            <AppButton
              variant="secondary"
              class="w-full"
              :loading="tripayForm.processing"
              @click="initiateTripay"
            >
        ...
        <div class="overflow-x-auto">
          ...
          >
            ...
            <template #cell-amount="{ row }"
              ><span class="font-bold">{{ money(row.amount) }}</span></template
            >
