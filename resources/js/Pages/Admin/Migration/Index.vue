...
const runDiscovery = async (force = false) => {
  ...
  try {
    ...
    discoveredSuffixes.value = Array.isArray(data.suffixes)
      ? data.suffixes
      : [];
...
const runLegacyTenantLoad = async (force = false) => {
  ...
  try {
    ...
    legacyTenants.value = Array.isArray(data.legacy_tenants)
      ? data.legacy_tenants
      : [];
  } catch (e) {
    legacyTenantsError.value =
      e?.message ?? "Gagal memuat daftar tenant legacy.";
  } finally {
...
const selectedLegacyTenant = computed(
  () =>
    legacyTenants.value.find(
      (item) => String(item.legacy_id) === String(form.legacy_id),
    ) ?? null,
);
    ...
    return [
      {
        value: "1",
        label: "Suffix 1",
        subtitle: "Default manual input (transaksi_1 / saldo_1)",
      },
    ];
  }
  return discoveredSuffixes.value.map((s) => {
    const countText =
      s.transaksi_count !== null
    ...
    return {
      ...
      badge:
        s.transaksi_count !== null
          ? `${s.transaksi_count.toLocaleString("id-ID")} Trx`
          : null,
    };
  });
});
...
const submitCutover = () => {
  if (!isExpertMode.value) {
    form.suffix = "";
    form.tenant_id =
      simpleFallbackMode.value === "manual" ? form.tenant_id : "";
...
watch(isExpertMode, () =>
  resetFormForMode(isExpertMode.value ? "expert" : "simple"),
);
    ...
    sseSource.addEventListener("update", (event) => {
      try {
        ...
        if (selectedRun.value) {
          if (data.output_log !== undefined)
            selectedRun.value.output_log = data.output_log;
          ...
          if (data.error_message !== undefined)
            selectedRun.value.error_message = data.error_message;
        }
  ...
  pollInterval = setInterval(() => {
    if (
      ...
      (selectedRun.value.status === "running" ||
        selectedRun.value.status === "pending")
    ) {
      fetchRunDetails(runId);
    } else {
      stopSseOrPolling();
...
const runColumns = [
  { key: "run_id", label: "ID Run" },
  { key: "tenant_name", label: "Tenant Target" },
  { key: "suffix", label: "Suffix" },
  { key: "mode", label: "Mode" },
  { key: "status", label: "Status" },
  { key: "started_at", label: "Waktu Mulai" },
...
<template>
  ...
  <AdminLayout>
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <AppCard>
            ...
            <form @submit.prevent="submitCutover" class="space-y-5">
              <div v-if="!isExpertMode" class="space-y-4">
                <div>
                  <SmartSelect
                    ...
                    required
                  />
                  <p v-if="legacyTenantsError" class="mt-2 text-sm text-error">
                    {{ legacyTenantsError }}
                  </p>
                </div>

                <div
                  v-if="selectedLegacyTenant"
                  class="space-y-3 rounded-xl border border-outline-variant p-4"
                >
                  <div
                  ...
                  >
                    <AppButton
                      type="button"
                      :variant="
                        simpleFallbackMode === 'auto' ? 'primary' : 'secondary'
                      "
                      size="compact"
              ...
              <div v-else class="space-y-5">
                    ...
                    >
                      <span class="text-on-surface-variant">
                        <template v-if="isDiscovering"
                          >Memindai database legacy… ({{
                            Math.round(70)
                          }}s)</template
                        >
                        <template v-else-if="discoveryError">
                          <span class="text-error"
                            >Gagal memindai: {{ discoveryError }}</span
                          >
                        </template>
                        ...
                        <template v-else>
                          Belum ada suffix terdeteksi — klik "Pindai Ulang DB
                          Legacy".
                        </template>
                      </span>
                      <AppButton
                ...
                <div
                  class="space-y-4 rounded-xl border border-outline-variant p-4"
                >
                  ...
                  <div class="space-y-3">
                    <span
                      ...
                      >Lompati Step (Optional Skipping):</span
                    >
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <AppSwitch
                        v-model="form.skip_fiscal"
                        label="Lompati Periode Fiskal"
                        description="Skip Fiscal Periods"
                      />
                      <AppSwitch
                        v-model="form.skip_coa"
                        label="Lompati Bagan Akun (COA)"
                        description="Skip COA Import"
                      />
                      <AppSwitch
                      ...
                      <AppSwitch
                        ...
                        label="Lompati Sequences"
                        description="Skip Sequences"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div
                class="flex justify-center border-t border-outline-variant pt-3"
              >
                <AppButton
                  type="button"
                  variant="ghost"
        ...
        <div>
          <AppCard>
            <template #header>
              <h2 class="text-lg font-bold text-primary">
                Petunjuk Alur Migrasi
              </h2>
            </template>
            <div
              class="space-y-3 text-xs text-on-surface-variant leading-relaxed"
            >
              <div
                class="rounded-lg border border-outline-variant bg-surface-container-low p-3 space-y-1"
              >
                <span class="block text-xs font-bold text-primary"
                  >Koneksi Database Legacy Aktif:</span
                >
                <p>
                  Host:
                  <code class="font-mono font-bold text-primary"
                    >{{ props.legacy_config?.host || "127.0.0.1" }}:{{
                      props.legacy_config?.port || 3306
                    }}</code
                  >
                </p>
                <p>
              ...
              <div class="flex items-start space-x-2">
                ...
                <p>
                  ...
                  <code
                    class="rounded bg-surface-container px-1 py-0.5 font-mono text-[11px] font-mono text-[11px] font-mono text-[11px] font-mono text-[11px] font-mono text-[11px]"
                    >.env</code
                  >.
                </p>
              </div>
              <div class="flex items-start space-x-2">
                ...
                <p>
                  Tabel legacy dibedakan oleh akhiran angka (Suffix ID), seperti
                  <code
                    class="rounded bg-surface-container px-1 py-0.5 font-mono text-[11px]"
                    >transaksi_1</code
                  >
              ...
              <div class="flex items-start space-x-2">
                ...
                <p>
                  Pilihlah <strong>Suffix Terdeteksi</strong> dari daftar
                  dropdown. Sistem akan otomatis memindai tabel yang tersedia.
                </p>
              </div>
              <div class="flex items-start space-x-2">
                <span class="shrink-0 font-bold text-secondary">4.</span>
                <p>
                  Disarankan melakukan <strong>Mode Dry-Run</strong> terlebih
                  dahulu untuk mensimulasikan validasi data tanpa mengubah
                  database utama.
                </p>
              </div>
            </div>
      ...
      <AppCard>
        <template #header>
          <div class="flex items-center justify-between">
            <h2 class="text-lg font-bold text-primary">
              Riwayat Eksekusi Migrasi (Cutover Runs)
            </h2>
            <AppButton
              variant="ghost"
              size="compact"
              icon="refresh"
              @click="router.reload({ only: ['runs'] })"
            >
              Refresh Data
            </AppButton>
          </div>
        ...
        <div class="overflow-x-auto">
          ...
          >
            <template #cell-run_id="{ row }"
              ...
              ></template
            >
            <template #cell-suffix="{ row }">
              <div class="space-y-1 font-mono text-on-surface-variant">
                <span>{{ row.suffix }}</span>
                <AppBadge
                  v-if="row.options?.legacy_name"
                  tone="neutral"
                  class="text-[10px]"
                  >Legacy: {{ row.options.legacy_name }}</AppBadge
                >
              </div>
            </template>
            <template #cell-mode="{ row }"
              ><AppBadge :tone="row.is_dry_run ? 'neutral' : 'warning'">{{
                row.is_dry_run ? "Dry Run" : "Live Cutover"
              }}</AppBadge></template
            >
            <template #cell-status="{ row }"
              ><AppBadge :tone="getStatusVariant(row.status)"
                ><span class="capitalize">{{ row.status }}</span></AppBadge
              ></template
            >
    ...
    <Teleport to="body">
      <div
        v-if="activeLogModal && selectedRun"
        class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm"
      >
        <div
          class="w-full max-w-4xl overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-2xl"
        >
          <div
            class="flex items-center justify-between border-b border-outline-variant p-4"
          >
            <div>
              <h3 class="text-lg font-bold text-primary">
                Monitoring Cutover Run #{{ selectedRun.id }} —
                {{ selectedRun.tenant_name }}
              </h3>
              <p class="text-xs text-on-surface-variant">
                Suffix: {{ selectedRun.suffix }} | Mode:
                {{ selectedRun.is_dry_run ? "Dry Run" : "Live" }}
              </p>
            </div>
            <AppButton
              variant="ghost"
          ...
          <div class="max-h-[80vh] overflow-y-auto space-y-4 p-4">
            ...
            <div
              v-if="selectedRun.steps && selectedRun.steps.length > 0"
              class="space-y-2"
            >
              <span
                class="text-xs font-bold uppercase tracking-wider text-primary"
                >Progress Tahapan Migrasi:</span
              >
              <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
                <div
                  v-for="step in selectedRun.steps"
                  :key="step.name"
                  class="rounded-lg border border-outline-variant p-2 text-xs"
                >
                  <div class="mb-1 flex items-center justify-between">
                    ...
                    >
                    <AppBadge
                      :tone="getStepStatusVariant(step.status)"
            ...
            <div class="relative space-y-1">
              <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-primary"
                  >Terminal Console Log Output:</span
                >
                <span
                  v-if="selectedRun.status === 'running'"
                  class="animate-pulse text-xs font-medium text-tertiary"
                >
                  Live Polling Output...
                </span>
              </div>
              <div class="relative">
                <pre
                  ref="logTerminal"
                  class="h-80 w-full overflow-y-auto rounded-lg border border-outline bg-surface-container-lowest p-4 font-mono text-xs text-secondary shadow-inner whitespace-pre-wrap"
                  @scroll="onLogScroll"
                  >{{
                    selectedRun.output_log || "Menunggu keluaran log..."
                  }}</pre>
                <AppButton
            ...
            <div
              ...
              class="rounded-lg bg-error-container p-3 text-xs text-error"
            >
          ...
          <div
            class="flex items-center justify-between border-t border-outline-variant p-4"
          >
            <div>
              <AppButton
            ...
            <AppButton variant="secondary" @click="closeLogModal"
              >Tutup Window Log</AppButton
            >
          </div>
        </div>
      </div>
