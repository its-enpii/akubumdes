...
const categoryOptions = [
  { value: "qris", label: "QRIS (Scan Langsung)", icon: "qr_code_scanner" },
  {
    ...
    label: "Virtual Account Bank (VA)",
    icon: "account_balance",
  },
];

const qrisChannels = computed(() =>
  props.channels.filter((c) => c.group === "qris" || c.code.includes("QRIS")),
);
const vaChannels = computed(() =>
  props.channels.filter(
    (c) => c.group === "virtual_account" || c.code.endsWith("VA"),
  ),
);
...
if (props.active_payment?.method_code) {
  ...
  selectedCategory.value = props.active_payment.method_code.includes("QRIS")
    ? "qris"
    : "virtual_account";
}
...
function checkStatus() {
  ...
  router.post(
    `/billing/invoices/${props.invoice.row_id}/check-status`,
    {},
    {
      preserveScroll: true,
      onFinish: () => {
      ...
      >
        <div>
          ...
          <div class="mt-3 flex flex-wrap items-center gap-3">
            ...
            <AppBadge :tone="tone(invoice.status)">{{
          ...
          <p class="mt-1 text-sm text-on-surface-variant">
            ...
            <span v-if="invoice.subscription">
              · Paket: {{ invoice.subscription.plan?.name }}</span
            >
          </p>
        </div>

        <div
          v-if="invoice.is_open && active_payment"
        ...
        <AppCard class="border-2 border-primary/20 bg-primary/5 p-6">
          ...
          >
            <div class="flex items-center gap-3">
              <span
                class="grid size-12 place-items-center rounded-xl bg-primary text-on-primary shadow-md"
              >
                <AppIcon
                  :name="
                    active_payment.method_code.includes('QRIS')
                      ? 'qr_code_scanner'
                      : 'account_balance'
                  "
                  size="28"
              ...
              <div>
                <span
                  ...
                  >Instruksi Pembayaran Aktif</span
                >
                <h2 class="text-xl font-bold text-on-surface">
                  {{ active_payment.payment_name }}
                </h2>
              </div>
            </div>
            <div class="text-right">
              ...
              <p class="font-bold text-error">
                {{ active_payment.expired_time || "24 Jam" }}
              </p>
            </div>
          </div>
          ...
          <div
            v-if="active_payment.qr_url"
            class="mt-6 flex flex-col items-center justify-center rounded-2xl bg-surface p-6 shadow-sm sm:flex-row sm:gap-8"
          >
            <div class="flex flex-col items-center">
              <div
                class="relative rounded-2xl border-4 border-white bg-white p-3 shadow-lg"
              >
                <img
            ...
            <div class="mt-6 max-w-md space-y-4 sm:mt-0">
              ...
              <div
                class="rounded-xl bg-surface-container-low p-4 text-xs text-on-surface space-y-2"
              >
                <p class="font-bold text-primary">Cara Pembayaran QRIS:</p>
                <ol class="list-decimal pl-4 space-y-1">
                  <li>
                    Buka aplikasi m-Banking (BCA, Mandiri, BRI, BNI, dll.) atau
                    e-Wallet (GoPay, OVO, Dana, ShopeePay).
                  </li>
                  <li>Pilih menu <strong>Scan / Bayar QRIS</strong>.</li>
                  <li>Arahkan kamera ke kode QR di layar ini.</li>
                  <li>
                    Periksa nama merchant dan nominal, lalu masukkan PIN Anda.
                  </li>
                </ol>
              </div>
              <div class="flex flex-wrap gap-2">
                ...
                <a
                  :href="active_payment.qr_url"
                  download="qris_invoice.png"
                  target="_blank"
                >
          ...
          <div
            v-else-if="active_payment.pay_code"
            class="mt-6 rounded-2xl bg-surface p-6 shadow-sm"
          >
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <div class="mt-1 flex items-center gap-3">
                    <span
                      class="font-mono text-2xl font-black tracking-wider text-primary"
                    >
                ...
                <div>
                  ...
                  <div class="mt-1 flex items-center gap-3">
                    ...
                    <AppButton
                      ...
                      icon="content_copy"
                      @click="
                        copyToClipboard(active_payment.total_amount, 'amount')
                      "
                    >
                      {{
                        copiedField === "amount"
              ...
              <div v-if="active_payment.instructions?.length" class="space-y-3">
                ...
                <div class="max-h-60 overflow-y-auto space-y-2 pr-2">
                  ...
                  >
                    ...
                    <ol
                      class="mt-1 list-decimal pl-4 text-xs text-on-surface-variant space-y-0.5"
                    >
                      <li
                        v-for="(step, sIdx) in inst.steps"
                        :key="sIdx"
                        v-html="step"
                      ></li>
                    </ol>
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        ...
        <AppCard class="lg:col-span-1 h-fit">
          ...
          <dl class="mt-4 divide-y divide-outline-variant text-sm">
            ...
            <div class="py-2.5 flex justify-between">
              <dt class="text-on-surface-variant font-semibold">
                Sisa Kewajiban
              </dt>
              <dd class="text-lg font-black text-primary">
                {{ money(invoice.remaining, invoice.currency) }}
              </dd>
            </div>
            <div class="py-2.5">
              <dt class="text-on-surface-variant">Keperluan</dt>
              <dd class="font-semibold text-primary">
                {{ purposeLabels[invoice.purpose] || invoice.purpose }}
              </dd>
            </div>
            <div class="py-2.5">
        ...
        <AppCard v-if="invoice.is_open" class="lg:col-span-2 space-y-6">
          ...
          <div v-if="selectedCategory === 'qris'" class="space-y-3">
            ...
            >
              <div class="flex items-center gap-3">
                <span
                  class="grid size-10 place-items-center rounded-lg bg-surface-container text-primary"
                >
                ...
                <div>
                  ...
                  <p class="text-xs text-on-surface-variant">
                    Semua Mobile Banking & E-Wallet (BCA, Mandiri, BRI, GoPay,
                    Dana, dll.)
                  </p>
                </div>
              </div>
          ...
          <div
            v-if="selectedCategory === 'virtual_account'"
          ...
          >
            ...
            >
              <div class="flex items-center gap-3">
                <span
                  class="grid size-9 place-items-center rounded-lg bg-surface-container text-primary font-bold text-xs"
                >
                  {{ ch.code.replace("VA", "") }}
                </span>
                <div>
        ...
        <AppCard
          ...
          class="lg:col-span-2 border border-secondary bg-secondary-container/30 p-6"
        >
          <div class="flex items-center gap-3">
            <AppIcon
              name="check_circle"
            ...
            <div>
              ...
              <p class="text-sm text-on-secondary-container">
                Pembayaran telah diterima{{
                  invoice.paid_at ? ` pada ${invoice.paid_at}` : ""
                }}. Masa langganan aktif.
              </p>
            </div>
          </div>
        </AppCard>
          ...
          >
            ...
            <template #cell-reference="{ row }">
              ...
              <span v-else class="font-mono text-xs">{{
                row.reference || row.tripay_reference || "—"
              }}</span>
            </template>
