...
const live = computed(() => {
  ...
  const pphCorporate =
    Math.max(profit, 0) * (Number(props.rates.corporate) / 100);
...
const orgName = computed(
  () => props.identity.short_name || props.identity.legal_name || "—",
);
</script>
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        <div>
          ...
          <p class="mt-1 text-on-surface-variant">
            Hitung PPh dari mutasi jurnal posted. Pilih akun pendapatan yang
            masuk basis pajak.
          </p>
        </div>
      ...
      <div
        v-if="error"
        class="rounded-xl border border-outline-variant bg-error-container px-4 py-3 text-sm text-on-error-container"
      >
        {{ error }}
      </div>
      ...
      <div class="grid gap-4 lg:grid-cols-3">
        <AppCard>
          ...
          <p class="mt-1 text-sm text-on-surface-variant">
            NPWP: {{ identity.tax_number || "Belum diisi (Pengaturan)" }}
          </p>
        </AppCard>
        <AppCard>
          <p
            class="text-xs font-bold uppercase tracking-wider text-on-surface-variant"
          >
            PPh Final {{ rates.final }}%
          </p>
          <p class="mt-2 text-2xl font-bold text-primary">
            {{ formatMoney(live.pph_final) }}
          </p>
          <p class="mt-1 text-xs text-on-surface-variant">
            Dari total pendapatan terpilih
          </p>
        </AppCard>
        <AppCard>
          <p
            class="text-xs font-bold uppercase tracking-wider text-on-surface-variant"
          >
            PPh Badan {{ rates.corporate }}%
          </p>
          <p class="mt-2 text-2xl font-bold text-primary">
            {{ formatMoney(live.pph_corporate) }}
          </p>
      ...
      <div class="grid gap-4 md:grid-cols-3">
        ...
        <AppCard>
          ...
          <p
            ...
            :class="live.profit >= 0 ? 'text-primary' : 'text-error'"
          >
      ...
      <AppCard :padded="false">
        ...
        >
          <div>
            ...
            <p class="text-sm text-on-surface-variant">
              Centang akun yang masuk perhitungan PPh.
            </p>
          </div>
          <div class="flex flex-wrap gap-2">
            <AppButton
              variant="secondary"
              size="compact"
              @click="selectAllNonZero"
              >Pilih non-nol</AppButton
            >
            <AppButton variant="ghost" size="compact" @click="clearSelection"
              >Kosongkan</AppButton
            >
          </div>
        </div>
        ...
        <div
          v-if="revenue_accounts.length === 0"
          class="px-6 py-8 text-sm text-on-surface-variant"
        >
        ...
        <div v-else class="divide-y divide-outline-variant">
          ...
          >
            ...
            <p class="text-right font-semibold text-primary md:text-left">
              {{ formatMoney(account.amount) }}
            </p>
            <div class="md:justify-self-end">
              <AppSwitch
                v-model="selected[account.row_id]"
                :label="`Sertakan ${account.code}`"
              />
            </div>
          </div>
        </div>
