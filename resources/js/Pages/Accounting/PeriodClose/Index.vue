...
const allowClose = computed(
  () => props.can_close && can("period_close.manage"),
);
...
const moneyDecimal = new Intl.NumberFormat("id-ID", {
...
async function closeMonth(month) {
  ...
  if (
    !(await confirmAction({
      title: "Tutup Periode",
      message: `Tutup periode ${String(month).padStart(2, "0")}/${props.year}? Jurnal draft harus kosong.`,
    ...
    return;
  router.post(
    `/accounting/period-close/${props.year}/${month}/close`,
    {},
    { preserveScroll: true },
  );
}
...
async function reopenMonth(month) {
  ...
  if (
    !(await confirmAction({
      ...
      message: `Buka kembali periode ${String(month).padStart(2, "0")}/${props.year}?`,
    ...
    return;
  router.post(
    `/accounting/period-close/${props.year}/${month}/reopen`,
    {},
    { preserveScroll: true },
  );
}
...
async function closeYear() {
  ...
  yearForm.post("/accounting/period-close/year-close", {
    preserveScroll: true,
  });
}
...
const villageTotal = computed(() =>
  Object.values(allocForm.villages).reduce((s, v) => s + n(v), 0),
);
const allocTotal = computed(
  () =>
    ...
    n(allocForm.retained),
);
const allocOver = computed(
  () => allocTotal.value - Number(props.allocation.remaining || 0) > 0.009,
);
...
async function submitAllocation() {
  ...
  if (allocTotal.value <= 0) {
    await showAlert({
      title: "Perhatian",
      message: "Isi minimal satu pos alokasi.",
    });
    return;
  }
  if (allocOver.value) {
    await showAlert({
      title: "Perhatian",
      message: "Total alokasi melebihi sisa laba.",
    });
    return;
  }
  if (
    !(await confirmAction({
      title: "Simpan Alokasi Laba",
      message: `Simpan alokasi laba ${props.year} total ${formatMoney(allocTotal.value)}?`,
      confirmLabel: "Simpan",
      variant: "primary",
    }))
  )
...
const periodColumns = [
  ...
  { key: "draft_journals", label: "Draft", align: "right" },
  { key: "closed_at", label: "Ditutup" },
  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        <div>
          ...
          <p class="mt-1 text-sm text-on-surface-variant">
            Tutup periode, bawa saldo akun ke tahun depan, lalu alokasi laba —
            satu langkah per tab.
          </p>
        </div>
        <div class="w-40">
          <SmartSelect
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <AppCard>
          ...
          <p
            ...
            :class="draft_journals > 0 ? 'text-error' : 'text-primary'"
          >
            {{ draft_journals }}
          </p>
        </AppCard>
        <AppCard>
          ...
          <p
            class="mt-2 text-2xl font-bold"
            :class="net_income < 0 ? 'text-error' : 'text-primary'"
          >
          ...
          >
            <template #cell-period="{ row }"
              ><span class="whitespace-nowrap text-on-surface-variant"
                >{{ formatPeriodDate(row.starts_at) }} –
                {{ formatPeriodDate(row.ends_at) }}</span
              ></template
            >
            <template #cell-status="{ row }"
              ><AppBadge :tone="statusTone[row.status] || 'neutral'">{{
                statusLabel[row.status] || row.status
              }}</AppBadge></template
            >
            <template #cell-draft_journals="{ row }"
              ><span
                :class="
                  row.draft_journals > 0 ? 'font-semibold text-error' : ''
                "
                >{{ row.draft_journals }}</span
              ></template
            >
      ...
      <AppCard v-show="tab === 'trial_balance'" class="overflow-hidden p-0">
        <div
          class="flex flex-wrap items-start justify-between gap-2 border-b border-outline-variant px-4 py-3"
        >
          <div>
            <h2 class="text-sm font-bold text-primary">
              Neraca Saldo per 31-12-{{ year }}
            </h2>
            <p class="text-xs text-on-surface-variant">
              Saldo akun pada akhir tahun buku — debit/kredit seimbang sebagai
              syarat tutup tahun.
            </p>
          </div>
        ...
        <div class="max-h-[28rem] overflow-auto">
          <ReportTable
            ...
            :rows="trial_balance.rows"
            row-key="row_id"
            size="compact"
            sticky-header
            empty-title="Belum ada akun dengan saldo."
          >
            ...
            <template #cell-ns_debit="{ row }">{{
              formatMoneyDecimal(row.ns_debit)
            }}</template>
            <template #cell-ns_credit="{ row }">{{
              formatMoneyDecimal(row.ns_credit)
            }}</template>
            <template #cell-lr_debit="{ row }">{{
              formatMoneyDecimal(row.lr_debit)
            }}</template>
            <template #cell-lr_credit="{ row }">{{
              formatMoneyDecimal(row.lr_credit)
            }}</template>
            <template #cell-bs_debit="{ row }">{{
              formatMoneyDecimal(row.bs_debit)
            }}</template>
            <template #cell-bs_credit="{ row }">{{
              formatMoneyDecimal(row.bs_credit)
            }}</template>
            <template #footer v-if="trial_balance.rows.length">
              <tr
                class="border-t-2 border-outline bg-surface-container-low font-semibold"
              >
                <td class="px-3 py-2">Jumlah</td>
                <td class="px-3 py-2 text-right tabular-nums">
                  {{ formatMoneyDecimal(trial_balance.totals.ns_debit) }}
                </td>
                <td class="px-3 py-2 text-right tabular-nums">
                  {{ formatMoneyDecimal(trial_balance.totals.ns_credit) }}
                </td>
                <td class="px-3 py-2 text-right tabular-nums">
                  {{ formatMoneyDecimal(trial_balance.totals.lr_debit) }}
                </td>
                <td class="px-3 py-2 text-right tabular-nums">
                  {{ formatMoneyDecimal(trial_balance.totals.lr_credit) }}
                </td>
                <td class="px-3 py-2 text-right tabular-nums">
                  {{ formatMoneyDecimal(trial_balance.totals.bs_debit) }}
                </td>
                <td class="px-3 py-2 text-right tabular-nums">
                  {{ formatMoneyDecimal(trial_balance.totals.bs_credit) }}
                </td>
              </tr>
            </template>
          </ReportTable>
        ...
        <p
          class="border-t border-outline-variant/40 px-4 py-2 text-xs text-on-surface-variant"
        >
          Laba/Rugi berjalan:
          <span class="font-semibold text-on-surface">{{
            formatMoneyDecimal(trial_balance.net_income)
          }}</span>
        </p>
        ...
        <div class="space-y-3 border-t border-outline-variant px-4 py-3">
          ...
          <p
            v-if="allowClose && !trial_balance.balanced"
      ...
      <AppCard v-show="tab === 'allocate'" class="overflow-hidden p-0">
        <div class="border-b border-outline-variant px-4 py-3">
          <h2 class="text-sm font-bold text-primary">
            Alokasi laba tahun {{ year }}
          </h2>
          <p class="text-xs text-on-surface-variant">
            Dr {{ allocation.accounts?.earnings?.code || "3.2.02.01" }} → Cr
            utang laba / laba ditahan. Tanggal biasanya 1 Jan tahun berikutnya
            (periode harus terbuka).
          </p>
        </div>

        <div v-if="allocation.error" class="px-4 py-6 text-sm text-error">
          {{ allocation.error }}
        </div>

        <template v-else>
          <div
            class="grid gap-3 border-b border-outline-variant p-4 sm:grid-cols-3"
          >
            <div>
              <p class="text-xs font-bold uppercase text-on-surface-variant">
                Laba tersedia
              </p>
              <p class="mt-1 text-xl font-bold text-primary">
                {{ formatMoney(allocation.available) }}
              </p>
            </div>
            <div>
              <p class="text-xs font-bold uppercase text-on-surface-variant">
              ...
              <p class="mt-1 text-xl font-bold">
                {{ formatMoney(allocation.already_allocated) }}
              </p>
            </div>
            <div>
              <p class="text-xs font-bold uppercase text-on-surface-variant">
              ...
              <p
                ...
                :class="allocation.remaining > 0 ? 'text-primary' : ''"
              >
          ...
          <div
            v-if="allocation.existing?.length"
            class="border-b border-outline-variant px-4 py-3"
          >
            <p class="mb-2 text-xs font-bold uppercase text-on-surface-variant">
              Jurnal alokasi sebelumnya
            </p>
          ...
          >
            ...
            >
              ...
              >
                <div>
                  <p
                    class="font-mono text-[11px] font-semibold tracking-wide text-on-surface-variant"
                  >
            ...
            >
              ...
              >
                ...
                <p class="text-sm tabular-nums text-on-surface-variant">
                  Î£
                  <span class="font-semibold text-primary">{{
                    formatMoney(villageTotal)
                  }}</span>
                </p>
              </div>
              <div
                class="max-h-80 divide-y divide-outline-variant/50 overflow-y-auto pr-1"
              >
                <div
            ...
            >
              <div class="text-sm">
                <p class="text-on-surface-variant">
                  Total alokasi
                  <span
                    class="ml-1 text-lg font-bold tabular-nums text-primary"
                  >
                  ...
                  <template v-else>
                    ...
                    {{
                      formatMoney(
                        Math.max(
                          0,
                          Number(allocation.remaining || 0) - allocTotal,
                        ),
                      )
                    }}
                  </template>
