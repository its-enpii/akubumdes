...
import { useMoney as useMoneyFormatter } from "../../../composables/useMoney.js";
...
const isInventory = computed(() =>
  ASSET_PURCHASE_TYPES.includes(currentType.value),
);
const sumberDanaLabel = computed(
  () => props.labels[currentType.value]?.sumber_dana ?? "Sumber Dana",
);
const disimpanKeLabel = computed(
  () => props.labels[currentType.value]?.disimpan_ke ?? "Disimpan Ke",
);
const sumberDanaOptions = computed(
  () => props.options[currentType.value]?.sumber_dana ?? [],
);
const disimpanKeOptions = computed(
  () => props.options[currentType.value]?.disimpan_ke ?? [],
);
...
function submit() {
  if (isInventory.value) {
    form.amount =
      acquisitionCost.value > 0 ? acquisitionCost.value : form.amount;
    if (!form.description) {
      const qty = Number(form.asset_quantity || 0);
...
function fetchHistory() {
  ...
  if (historyForm.value.period === "daily")
    params.date = historyForm.value.date;
  else if (historyForm.value.period === "monthly")
    params.month = historyForm.value.month;
  else if (historyForm.value.period === "yearly")
    params.year = historyForm.value.year;
...
const flashEntry = computed(() =>
  typeof flash.value === "object" ? flash.value : null,
);
const flashLines = computed(() => flashEntry.value?.lines ?? []);
const debitedLines = computed(() =>
  flashLines.value.filter((line) => line.debit > 0),
);
const creditedLines = computed(() =>
  flashLines.value.filter((line) => line.credit > 0),
);
...
const historyColumns = [
  { key: "row_sequence", label: "Nomor" },
  { key: "transaction_date", label: "Tanggal" },
  { key: "entry_id", label: "Trx ID" },
  { key: "description", label: "Keterangan" },
  { key: "debit", label: "Debit", align: "right" },
  { key: "credit", label: "Kredit", align: "right" },
  { key: "running_balance", label: "Saldo", align: "right" },
];
...
function transactionTypeLabel(value) {
  return (
    props.transactionTypes.find((type) => type.value === value)?.label ??
    value ??
    "—"
  );
}
...
const historySummary = computed(() => {
  ...
  const totalDebit = historyRows.value.reduce(
    (sum, row) => sum + (row.debit || 0),
    0,
  );
  const totalCredit = historyRows.value.reduce(
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      <header>
        ...
        <p class="mt-1 text-on-surface-variant">
          Catat transaksi jurnal manual dengan satu akun debit dan satu akun
          kredit.
        </p>
      </header>
      ...
      <AppCard v-if="flashEntry">
        <div class="mb-5 flex items-start gap-3">
          <div
            class="grid size-12 shrink-0 place-items-center rounded-full bg-secondary-container text-secondary"
          >
          ...
          <div class="flex-1">
            ...
            <p class="mt-1 text-sm text-on-surface-variant">
              <span class="font-semibold">{{
                flashEntry.entry.transaction_date
              }}</span>
              <span v-if="flashEntry.entry.journal_number">
                · No. {{ flashEntry.entry.journal_number }}</span
              >
              · {{ transactionTypeLabel(flashEntry.entry.transaction_type) }}
            </p>
            <p
              v-if="flashEntry.entry.description"
              class="mt-1 text-sm text-on-surface-variant"
            >
              {{ flashEntry.entry.description }}
            </p>
          </div>
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div
            class="rounded-xl border border-secondary/30 bg-secondary-container/40 p-4"
          >
            ...
            <ul class="space-y-2">
              <li
                v-for="line in debitedLines"
                :key="`d-${line.account_code}`"
                class="rounded-lg bg-surface-container-lowest p-3"
              >
                <p class="text-sm font-bold text-primary">
                  {{ line.account_code }} · {{ line.account_name }}
                </p>
                <p class="mt-1 text-base font-bold text-secondary">
                  {{ currency(line.debit) }}
                </p>
              </li>
              <li
                v-if="!debitedLines.length"
                class="text-sm text-on-surface-variant"
              >
                Tidak ada akun debit.
              </li>
            </ul>
          </div>
          <div
            class="rounded-xl border border-error/30 bg-error-container/40 p-4"
          >
            <div class="mb-3 flex items-center gap-2">
            ...
            <ul class="space-y-2">
              <li
                v-for="line in creditedLines"
                :key="`c-${line.account_code}`"
                class="rounded-lg bg-surface-container-lowest p-3"
              >
                <p class="text-sm font-bold text-primary">
                  {{ line.account_code }} · {{ line.account_name }}
                </p>
                <p class="mt-1 text-base font-bold text-error">
                  {{ currency(line.credit) }}
                </p>
              </li>
              <li
                v-if="!creditedLines.length"
                class="text-sm text-on-surface-variant"
              >
        ...
        <form class="space-y-5" @submit.prevent="submit">
          ...
          <template v-if="isInventory">
            <div class="grid gap-4 sm:grid-cols-3">
              ...
              <AppInput
                ...
                min="1"
                required
                :error="form.errors.asset_quantity"
                placeholder="1"
              />
            </div>
            <div class="grid gap-4 sm:grid-cols-3">
              <AppCurrencyInput
                v-model="form.asset_unit_cost"
                label="Harga Satuan"
                icon="payments"
                :min="1"
                required
                :error="form.errors.asset_unit_cost"
                placeholder="0"
              />
              <AppInput
                ...
                :placeholder="
                  currentType === 'pembelian_aset_tanah' ? '0' : '48'
                "
                :hint="
                  currentType === 'pembelian_aset_tanah'
                    ? 'Tanah: 0 = tidak disusutkan'
                    : null
                "
              />
              <AppCurrencyInput
                ...
                placeholder="0"
                hint="Otomatis: unit × harga satuan"
              />
            </div>
          </template>
          ...
          <template v-else>
            <AppInput
              ...
              :error="form.errors.reference"
              placeholder="No referensi / catatan tambahan"
            />
            <AppCurrencyInput
              v-model="form.amount"
              label="Nominal"
          ...
          >
            <a :href="pagePath"
              ><AppButton variant="secondary" type="button">Reset</AppButton></a
            >
            <AppButton
              type="submit"
      ...
      <AppCard>
        <template #header>
          <div>
            ...
            <p class="mt-1 text-sm text-on-surface-variant">
              Lihat pergerakan debit/kredit suatu akun pada periode tertentu.
            </p>
          </div>
        </template>
        ...
        <form class="space-y-5" @submit.prevent="fetchHistory">
          <div class="grid gap-4 lg:grid-cols-3">
            <SmartSelect
              v-model="historyForm.account_row_id"
            ...
            <AppDatePicker
              ...
              v-model="periodValue"
              :mode="
                historyForm.period === 'daily'
                  ? 'date'
                  : historyForm.period === 'monthly'
                    ? 'month'
                    : 'year'
              "
          ...
          >
            <AppButton
              ...
              >Reset</AppButton
            >
            <AppButton type="submit" :loading="historyLoading" icon="search"
              >Tampilkan</AppButton
            >
          </div>
        </form>
        ...
        <AppEmptyState
          v-if="!props.history?.account"
          class="mt-6"
          icon="database"
          title="Pilih akun & periode"
          description="Pilih akun dan rentang waktu untuk melihat pergerakan debit/kredit."
        />
      </AppCard>
    </div>
    ...
    <AppModal
      ...
      :title="`Riwayat · ${props.history?.account?.code ?? ''}`"
      size="full"
      @update:model-value="closeHistoryModal"
    >
      <div v-if="props.history?.account" class="space-y-4">
        <p class="text-sm text-on-surface-variant">
          {{ props.history.account.name }} ·
          {{ props.history.account.account_type }} · Saldo Normal
          {{
            props.history.account.normal_balance === "D" ? "Debit" : "Kredit"
          }}
        </p>
        <p class="text-xs text-on-surface-variant">
          {{ periodDetail
          }}<span v-if="historyRange">
            · {{ historyRange.start }} → {{ historyRange.end }}</span
          >
        </p>
        ...
        <div v-if="historySummary" class="grid gap-3 sm:grid-cols-4">
          ...
          >
            ...
            <p class="mt-1 text-xl font-bold text-secondary">
              {{ currency(historySummary.totalDebit) }}
            </p>
          </div>
          <div
            class="rounded-xl border border-outline-variant bg-surface-container-lowest p-4"
          >
            ...
            <p class="mt-1 text-xl font-bold text-error">
              {{ currency(historySummary.totalCredit) }}
            </p>
          </div>
          <div
            class="rounded-xl border border-outline-variant bg-surface-container-lowest p-4"
          >
            ...
            <p class="mt-1 text-xl font-bold text-primary">
              {{ currency(historySummary.closingBalance) }}
            </p>
          </div>
        </div>
        ...
        >
          <ReportTable
            ...
            size="compact"
          >
            <template #cell-row_sequence="{ row }"
            ...
            <template #cell-entry_id="{ row }">
              <a
                v-if="row.entry_public_id"
                :href="entryHref(row.entry_public_id)"
        ...
        <AppEmptyState
          ...
          :description="`Tidak ada jurnal terposting pada akun ini di periode ${periodDetail}.`"
        />
      </div>
    </AppModal>
  </AuthenticatedLayout>
