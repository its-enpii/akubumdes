...
const pdfHref = computed(() => {
  const q = new URLSearchParams({
    ...
    month:
      props.filters.month === null || props.filters.month === undefined
        ? "all"
    ...
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        ...
        <a :href="pdfHref" target="_blank" rel="noopener">
          <AppButton variant="secondary" icon="picture_as_pdf" size="compact"
            >PDF</AppButton
          >
        </a>
      </div>
      ...
      <AppCard class="p-5">
        ...
        <p v-if="identity.address" class="text-sm text-on-surface-variant">
          {{ identity.address }}
        </p>
        <p
          v-if="identity.registration_number || identity.tax_number"
          class="mt-1 text-xs text-on-surface-variant"
        >
          <span v-if="identity.registration_number"
            >Registrasi {{ identity.registration_number }}</span
          >
          <span v-if="identity.registration_number && identity.tax_number">
            ·
          </span>
      ...
      <AppCard class="p-5">
        <h2
          class="mb-3 text-sm font-bold uppercase tracking-wide text-on-surface-variant"
        >
