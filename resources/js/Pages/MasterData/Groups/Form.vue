...
function option(item) {
  return { value: item.row_id, label: item.name };
}
...
async function searchMembers(search = "") {
  ...
  try {
    const response = await fetch(
      `${path}/member-options?search=${encodeURIComponent(query)}&exclude=${encodeURIComponent(exclude)}`,
      { headers: { Accept: "application/json" }, signal: controller.signal },
    );
    if (!response.ok) throw new Error("Data anggota gagal dimuat.");
    const payload = await response.json();
    const selected = new Map(
      selectedMembers.value.map((member) => [String(member.value), member]),
    );
    memberOptions.value = [
      ...selected.values(),
      ...payload.data.filter((member) => !selected.has(String(member.value))),
    ];
    memberSearchEmpty.value = Boolean(query) && payload.data.length === 0;
  } catch (error) {
    if (error.name !== "AbortError")
      memberOptions.value = [...selectedMembers.value];
  } finally {
...
async function storeQuickMember() {
  ...
  try {
    ...
    if (
      !memberOptions.value.some(
        (item) => String(item.value) === String(member.value),
      )
    )
      memberOptions.value.unshift(member);
    if (
      !selectedMembers.value.some(
        (item) => String(item.value) === String(member.value),
      )
    )
    ...
    if (error.response?.status === 422) {
      quickErrors.value = Object.fromEntries(
...
function addMember() {
  const member = memberOptions.value.find(
    (item) => String(item.value) === String(candidateId.value),
  );
  if (
    !member ||
    selectedMembers.value.some(
...
function submit() {
  editing ? form.put(`${path}/${props.group.row_id}`) : form.post(path);
}
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-7xl">
      <header class="mb-6">
        <Link :href="path" class="text-sm font-semibold text-primary"
          >← Kembali ke daftar kelompok</Link
        >
        <h1 class="mt-3 text-2xl font-bold text-primary">
          {{ editing ? "Edit Kelompok" : "Tambah Kelompok" }}
        </h1>
        <p class="mt-1 text-on-surface-variant">
          Kelola identitas, anggota, dan pengurus kelompok.
        </p>
      </header>
      <AppCard>
        <form class="space-y-5" @submit.prevent="submit">
          <section>
            ...
            <div
              ...
              :class="editing ? 'xl:grid-cols-4' : 'xl:grid-cols-3'"
            >
              <AppInput
                v-if="editing"
                :model-value="group.code"
                label="Kode Kelompok"
                icon="tag"
                readonly
              />
              ...
              <AppDatePicker
                ...
                :max="today"
                clearable
                :error="form.errors.established_at"
              />
            </div>
            <div class="mt-4 grid gap-4 xl:grid-cols-3">
              <AppInput
                ...
                label="Alamat"
                icon="home"
                placeholder="Masukkan alamat lengkap kelompok"
          ...
          <section class="border-t border-outline-variant pt-4">
            ...
            <div class="mt-3 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              />
              <SmartSelect
                v-model="form.group_level_id"
                label="Tingkatan"
                :options="levelOptions"
                required
                :error="form.errors.group_level_id"
              />
              <SmartSelect
                v-model="form.group_function_id"
                label="Fungsi Kelompok"
                :options="functionOptions"
                required
                :error="form.errors.group_function_id"
              />
            </div>
          </section>
          ...
          <section class="border-t border-outline-variant pt-4">
            ...
            <p class="mt-1 text-sm text-on-surface-variant">
              Tambahkan minimal tiga anggota aktif.
            </p>
            <div class="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end">
              <div class="flex-1">
                <SmartSelect
                  ...
                  :empty-action-label="
                    memberSearchEmpty && memberSearch
            ...
            <p v-if="memberError" class="mt-2 text-sm text-error">
              {{ memberError }}
            </p>
            <div
              class="mt-3 overflow-x-auto rounded-xl border border-outline-variant"
            >
              <ReportTable
          ...
          <section class="border-t border-outline-variant pt-4">
            ...
            <p class="mt-1 text-sm text-on-surface-variant">
              Ketua, sekretaris, dan bendahara wajib berbeda.
            </p>
            <div class="mt-3 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              <SmartSelect
                ...
                :excluded-values="
                  [form.secretary_id, form.treasurer_id].filter(Boolean)
                "
                searchable
                required
                :error="form.errors.chair_id"
              />
              <SmartSelect
                ...
                :options="selectedOptions"
                :excluded-values="
                  [form.chair_id, form.treasurer_id].filter(Boolean)
                "
                searchable
                required
                :error="form.errors.secretary_id"
              />
              <SmartSelect
                ...
                :options="selectedOptions"
                :excluded-values="
                  [form.chair_id, form.secretary_id].filter(Boolean)
                "
                searchable
                required
                :error="form.errors.treasurer_id"
              />
            </div>
          </section>
          ...
          <div class="flex justify-end gap-3">
            <Link :href="path"
              ><AppButton variant="secondary">Batal</AppButton></Link
    ...
    <AppModal
      ...
      :closeable="!quickSaving"
    >
      <form
        id="quick-member-form"
        class="space-y-4"
        @submit.prevent="storeQuickMember"
      >
        <p class="text-sm text-on-surface-variant">
          Lengkapi identitas ringkas. Tanggal terdaftar, alamat, dan status
          ditentukan otomatis.
        </p>
        <div class="grid gap-4 sm:grid-cols-2">
          <AppInput
            v-model="quickMember.nik"
            label="NIK"
            icon="badge"
            inputmode="numeric"
            maxlength="16"
            required
            autofocus
            :error="quickErrors.nik"
          />
          <AppInput
            v-model="quickMember.name"
            label="Nama"
            icon="person"
            required
          ...
          />
          <AppRadioGroup
            v-model="quickMember.gender"
            label="Jenis Kelamin"
            :options="genderOptions"
            required
            :error="quickErrors.gender"
          />
          <SmartSelect
            v-model="quickMember.village_id"
            label="Desa"
            :options="villageOptions"
            searchable
            required
            :error="quickErrors.village_id"
          />
        </div>
        <p
          v-if="quickError"
