...
function onIdentityPhotoChange(event) {
  ...
  identityPhotoForm.post(
    `/master-data/members/${props.member.row_id}/identity-photo`,
    {
      preserveScroll: true,
      onSuccess: () => {
    ...
    <div class="mx-auto max-w-7xl space-y-6">
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <Link
            ...
            :href="`/master-data/members/${member.row_id}/edit`"
          >
            <AppButton variant="secondary" icon="edit" size="compact"
              >Edit</AppButton
            >
          </Link>
          <AppButton
      ...
      <div class="grid gap-6 lg:grid-cols-5">
        <AppCard class="lg:col-span-2 p-5">
          <h2
            class="text-sm font-bold uppercase tracking-wide text-on-surface-variant"
          >
            Profil
          </h2>
          <dl class="mt-4 space-y-3 text-sm">
            ...
            <div class="flex justify-between gap-4">
              ...
              <dd class="text-right font-medium">
                {{ member.birth_place || "—" }}
                <span v-if="member.birth_date">
                  · {{ formatDate(member.birth_date) }}</span
                >
              </dd>
            </div>
            <div class="flex justify-between gap-4">
          ...
          <div
            v-if="member.business?.name"
            class="mt-6 border-t border-outline-variant pt-4"
          >
            <h3
              class="text-xs font-bold uppercase tracking-wide text-on-surface-variant"
            >
              Usaha
            </h3>
            ...
            <p
              v-if="member.business.description"
          ...
          >
            ...
            <p class="text-sm text-on-surface-variant">
              ...
              <span v-if="member.guarantor.nik">
                · NIK {{ member.guarantor.nik }}</span
              >
            </p>
          </div>

          <div
            v-if="(member.groups || []).length"
            class="mt-6 border-t border-outline-variant pt-4"
          >
            <h3
              class="text-xs font-bold uppercase tracking-wide text-on-surface-variant"
        ...
        <AppCard class="lg:col-span-3">
          ...
          <p class="mt-1 text-sm text-on-surface-variant">
            Foto FC KTP digunakan pada dokumen keanggotaan.
          </p>

          <div class="mt-4 grid gap-4 md:grid-cols-[10rem_1fr] md:items-start">
            <div
              class="grid h-[5.4rem] place-items-center overflow-hidden rounded-lg border border-outline-variant bg-surface-container-lowest text-center"
            >
              <img
              ...
              <AppIcon
                v-else
                name="badge"
                class="text-3xl text-on-surface-variant"
              />
            </div>
            ...
            <form class="space-y-3" @submit.prevent>
              ...
              <p class="text-sm">
                ...
                {{ member.nik || "Belum tersedia" }}
              </p>
              <p
                v-if="identityPhotoForm.errors.identity_photo"
              ...
              <div
                class="flex justify-end border-t border-outline-variant pt-3"
              >
                <AppButton
