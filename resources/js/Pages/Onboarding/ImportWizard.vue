...
const balanceLines = ref(
  props.accounts.map((acc) => {
    const line = props.existingOpening?.lines?.find(
      (l) => l.account_row_id === acc.row_id,
    );
    return {
...
const asOfDate = ref(
  props.existingOpening?.as_of_date || new Date().toISOString().split("T")[0],
);
...
const totalDebit = computed(() => {
  return balanceLines.value.reduce(
...
function loadManualOpeningForYear(year) {
  ...
  manualLines.value = props.accounts.map((acc) => {
    const found = existing.find(
      (o) => Number(o.account_row_id) === Number(acc.row_id),
    );
    return {
      account_row_id: acc.row_id,
      code: acc.code,
...
const manualIsBalanced = computed(
  () =>
    Math.abs(manualTotalDebit.value - manualTotalCredit.value) < 0.01 &&
    manualTotalDebit.value > 0,
);
...
const submitManualOpening = () => {
  ...
  manualOpeningForm.post(
    `${baseUrl.value}/onboarding/opening-balances/manual`,
    {
      preserveScroll: true,
    },
  );
...
function pickAccountForAggregateLine(idx, option) {
  ...
  const acc = props.accounts.find(
    (a) => Number(a.row_id) === Number(option.value ?? option.row_id ?? option),
  );
  if (!acc) return;
...
const aggregateIsBalanced = computed(
  () =>
    Math.abs(aggregateTotalDebit.value - aggregateTotalCredit.value) < 0.01 &&
    aggregateTotalDebit.value > 0,
);
...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-7xl space-y-6">
      <!-- Header Banner -->
      <header
        class="rounded-2xl bg-gradient-to-r from-primary to-primary-deep p-6 text-on-primary shadow-xl"
      >
        <div
        ...
        >
          <div>
            <span
              class="inline-block rounded-full bg-primary-container px-3 py-1 text-xs font-semibold uppercase tracking-wider text-on-primary-container"
            >
      ...
      <div v-if="activeTab === 'balances'" class="space-y-6">
        <AppCard>
            ...
            >
              <div>
                <h2 class="text-lg font-semibold text-primary">
                  Form Neraca Saldo Awal Akun COA
                </h2>
                <p class="text-xs text-on-surface-variant">
                  Masukkan nilai saldo pada tanggal awal konversi operasional
                  aplikasi.
                </p>
              </div>
              <div class="flex items-center space-x-3">
                <label class="text-xs font-semibold text-on-surface"
                  >Tanggal Saldo Awal:</label
                >
                <AppDatePicker
                  v-model="asOfDate"
          ...
          <div
            class="mb-6 rounded-xl border p-4 shadow-sm"
            :class="
              isBalanced
            ...
            >
              <div class="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p class="text-xs text-on-surface-variant">
                    Total Debit:
                    <strong>Rp {{ formatMoney(totalDebit) }}</strong> | Total
                    Kredit: <strong>Rp {{ formatMoney(totalCredit) }}</strong>
                  </p>
                </div>
              </div>
              <div>
      ...
      >
        <AppCard class="hover:border-primary transition-colors">
          ...
          <p class="text-xs text-on-surface-variant mt-1">
            Format neraca saldo awal akun COA debit/kredit.
          </p>
          <a
            :href="`${baseUrl}/onboarding/templates/saldo-awal`"
        ...
        <AppCard class="hover:border-primary transition-colors">
          ...
          <a
            :href="`${baseUrl}/onboarding/templates/aset-tetap`"
            class="mt-4 inline-block text-xs font-semibold text-secondary"
            >?? Download CSV Template</a
          >
        </AppCard>
        ...
        <AppCard>
          <template #header>
            ...
            >
              <div>
                <h2 class="text-lg font-semibold text-primary">
                  Saldo Awal per Tahun Fiskal (Manual)
                </h2>
                <p class="text-xs text-on-surface-variant max-w-2xl">
                  Tulis langsung ke tabel
                  <code class="font-mono">account_opening_balances</code> per
                  tahun fiskal. Cocok untuk tenant baru yang join di tengah
                  tahun (mis. tahu saldo 1 Jan lalu backfill jurnal Jan-Mei
                  terpisah di tab berikutnya).
                </p>
              </div>
              <div class="flex items-center space-x-3">
                <label class="text-xs font-semibold text-on-surface"
          ...
          <div
            v-if="manualHasExisting"
            class="mb-6 rounded-xl border p-4 shadow-sm bg-tertiary-fixed border-tertiary"
          >
            <div class="flex items-center space-x-3">
              ...
              <div>
                ...
                <p class="text-xs text-on-surface-variant">
                  Menyimpan akan memperbarui nilai existing (preserve source).
                  Tidak menimpa source='migration' (legacy).
                </p>
              </div>
            </div>
          </div>
            ...
            >
              <div class="flex items-center space-x-4">
                ...
                <div>
                  <h3 class="text-sm font-bold text-primary">
                    Status:
                    {{ manualIsBalanced ? "SEIMBANG" : "BELUM SEIMBANG" }}
                  </h3>
                  <p class="text-xs text-on-surface-variant">
                    Total Debit:
                    <strong>Rp {{ formatMoney(manualTotalDebit) }}</strong> |
                    Total Kredit:
                    <strong>Rp {{ formatMoney(manualTotalCredit) }}</strong>
                  </p>
                </div>
              </div>
              <div>
                <AppBadge :tone="manualIsBalanced ? 'success' : 'warning'">
                  Selisih: Rp
                  {{
                    formatMoney(Math.abs(manualTotalDebit - manualTotalCredit))
                  }}
                </AppBadge>
              </div>
            </div>
          ...
          <div class="flex justify-end pt-6">
            ...
            >
              ...
              <span v-else
                >💾 Simpan Saldo Awal Tahun {{ manualFiscalYear }}</span
              >
            </AppButton>
          </div>
        </AppCard>
      ...
      <div v-if="activeTab === 'aggregate-journal'" class="space-y-6">
        <AppCard>
          <template #header>
            <div>
              <h2 class="text-lg font-semibold text-primary">
                Jurnal Agregat Mid-Year (Backfill Banyak Bulan)
              </h2>
              <p class="text-xs text-on-surface-variant max-w-3xl">
                Posting 1 jurnal <strong>multi-line</strong> untuk backfill
                agregat transaksi antar tanggal opening balance dan tanggal join
                (mis. Jan-Mei). Saldo akhir bulan = Opening + Jurnal Agregat +
                jurnal reguler bulanan.
              </p>
            </div>
          </template>

          <div
            class="rounded-xl border p-4 bg-tertiary-fixed border-tertiary mb-6"
          >
            <div class="flex items-start space-x-3">
              ...
              <div class="text-xs text-on-surface">
                ...
                <ol class="list-decimal pl-5 mt-1 space-y-1">
                  <li>
                    Set <code>transaction_date</code> = tanggal join (mis.
                    2026-06-01).
                  </li>
                  <li>
                    Tambah baris untuk setiap akun yang berubah (Kas debit,
                    Modal/Pendapatan credit, dll).
                  </li>
                  <li>Total Debit HARUS = Total Kredit.</li>
                  <li>
                    Submit — jurnal akan di-post lewat
                    <code>JournalPostingService</code> (validasi period open
                    otomatis).
                  </li>
                </ol>
              </div>
