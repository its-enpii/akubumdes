...
<template>
    <div class="space-y-4">
        ...
        <div class="relative overflow-x-auto">
            ...
            <table class="w-full text-left"><thead class="border-b-2 border-primary bg-surface-container-low"><tr><th v-for="column in columns" :key="column.key" class="px-6 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-on-surface-variant" :class="column.class"><button v-if="column.sortable" type="button" class="inline-flex items-center gap-1 transition-colors hover:text-primary" @click="sortBy(column)">{{ column.label }}<AppIcon :name="sort === column.key ? direction === 'asc' ? 'arrow_upward' : 'arrow_downward' : 'unfold_more'" class="text-lg text-outline" /></button><span v-else>{{ column.label }}</span></th><th v-if="$slots.actions" class="px-6 py-3 text-right text-[11px] font-semibold uppercase tracking-[0.08em] text-on-surface-variant">Aksi</th></tr></thead><tbody><tr v-for="(row, index) in rows" :key="row.row_id || row.id || index" class="border-t border-outline-variant transition-colors duration-200 hover:bg-surface-container-low"><td v-for="column in columns" :key="column.key" class="px-6 py-3.5" :class="column.class"><slot :name="`cell-${column.key}`" :row="row" :value="row[column.key]">{{ row[column.key] ?? '—' }}</slot></td><td v-if="$slots.actions" class="px-6 py-3.5 text-right"><slot name="actions" :row="row" /></td></tr></tbody></table>
